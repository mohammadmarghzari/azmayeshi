...
- ریسک: **{stats['سالانه']['vol']*100:.2f}%**
سه‌ماهه:
- سود: **{stats['سه‌ماهه']['return']*100:.2f}%**
- ریسک: **{stats['سه‌ماهه']['vol']*100:.2f}%**
دوماهه:
- سود: **{stats['دوماهه']['return']*100:.2f}%**
- ریسک: **{stats['دوماهه']['vol']*100:.2f}%**
یک‌ماهه:
- سود: **{stats['یک‌ماهه']['return']*100:.2f}%**
...
نسبت سورتینو: **{stats['sortino']:.2f}**
    ...
    # مرز کارا مونت‌کارلو و CVaR
    st.subheader("🌈 مرز کارا پرتفوها (مونت‌کارلو)")
    fig_mc = go.Figure()
    fig_mc.add_trace(go.Scatter(
        x=results[1]*100, y=results[0]*100,
        mode='markers', marker=dict(size=6, color=results[2], colorscale='Viridis', colorbar=dict(title='Sharpe Ratio')),
        name="پرتفوهای شبیه‌سازی‌شده"
    ))
    fig_mc.add_trace(go.Scatter(
        x=[np.sqrt(np.dot(best_weights.T, np.dot(cov_matrix, best_weights)))*100],
        y=[np.dot(best_weights, mean_returns)*100], mode='markers+text',
        marker=dict(size=18, color='red', symbol='star'),
        name="بهینه مونت‌کارلو", text=["⭐"], textposition="top center"
    ))
    fig_mc.update_layout(
        title="مرز کارا (سبک مونت‌کارلو)",
        xaxis_title="ریسک سالانه (%)", yaxis_title="بازده سالانه (%)",
        font_family="Vazirmatn", title_font_size=22
    )
    st.plotly_chart(fig_mc, use_container_width=True)

    st.subheader("🌈 مرز کارا پرتفوها (CVaR)")
    fig_cvar = go.Figure()
    fig_cvar.add_trace(go.Scatter(
        x=results[1]*100, y=results[0]*100,
        mode='markers', marker=dict(size=6, color=-results[4], colorscale='Blues', colorbar=dict(title='-CVaR')),
        name="پرتفوهای شبیه‌سازی‌شده"
    ))
    fig_cvar.add_trace(go.Scatter(
        x=[np.sqrt(np.dot(cvar_weights.T, np.dot(cov_matrix, cvar_weights)))*100],
        y=[np.dot(cvar_weights, mean_returns)*100], mode='markers+text',
        marker=dict(size=18, color='orange', symbol='star'),
        name="بهینه CVaR", text=["⭐"], textposition="top center"
    ))
    fig_cvar.update_layout(
        title="مرز کارا (سبک CVaR)",
        xaxis_title="ریسک سالانه (%)", yaxis_title="بازده سالانه (%)",
        font_family="Vazirmatn", title_font_size=22
    )
