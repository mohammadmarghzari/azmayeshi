...
def effective_hedged_vol(raw_returns_series, K_put, S, premium_per_share):
    """
    نوسان مؤثر یک دارایی پس از اعمال Protective Put را محاسبه می‌کند.
    منطق: بازده‌هایی که زیر سطح Put (K/S - 1) هستند را به floor محدود می‌کنیم،
    سپس هزینه پرمیوم روزانه را از همه بازده‌ها کم می‌کنیم.
    این روش اصولی‌ترین راه برای نشان دادن کاهش ریسک واقعی است.
    """
    floor_ret = (K_put / S) - 1.0          # حداقل بازده با Put
    cost_daily = (premium_per_share / S) / 365.0  # هزینه روزانه بیمه
...
def apply_hedge_to_returns(returns_df, ticker, K_put, S, premium_per_share):
    """
    یک کپی از DataFrame بازده‌ها برمی‌گرداند که در آن
    بازده‌های ticker با بازده‌های هج‌شده جایگزین شده‌اند.
    """
    df = returns_df.copy()
...
if st.session_state.get("cc_result"):
    ...
    for ok, txt in checks:
        icon = "✓" if ok else "✗"
        row_c = green_c if ok else red_c
        txt_c = "var(--silver)" if ok else "var(--muted)"
        cc_check_rows += (
            f"<div style='display:flex;align-items:flex-start;gap:8px;padding:0.25rem 0;"
            f"border-bottom:1px solid var(--border2)'>"
            f"<span style='color:{row_c};font-size:0.85rem;min-width:16px;margin-top:1px'>{icon}</span>"
            f"<span style='font-size:0.75rem;color:{txt_c};line-height:1.5'>{txt}</span>"
            f"</div>"
        )
    st.markdown(
        f"<div style='background:var(--card);border:1px solid var(--border2);"
        f"border-radius:4px;padding:0.9rem 1.1rem'>"
        f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:0.7rem'>"
        f"<span style='font-size:0.62rem;font-weight:700;letter-spacing:0.12em;"
        f"color:var(--muted);text-transform:uppercase'>شرایط کلی</span>"
        f"<span style='font-family:JetBrains Mono,monospace;font-weight:700;"
        f"font-size:0.9rem;color:{score_color}'>{passed}/{len(checks)} ✓</span>"
        f"</div>"
        f"{cc_check_rows}"
        f"<div style='margin-top:0.6rem;font-size:0.65rem;color:var(--muted);line-height:1.6'>"
        f"💡 CC ایده‌آل: OTM · IV بالا · Delta ۰.۲–۰.۳۵ · ۲۱–۴۵ روز تا انقضا · روند سهام خنثی یا کمی صعودی"
        f"</div></div>",
        unsafe_allow_html=True
    )
...
with tab2:
    ...
    if m is None or w is None:
        ...
        # ── محاسبه متریک‌های هج‌شده (اگر هج فعال است) ──────────────────────
        _ha_tab2 = st.session_state.get("hedged_assets", {})
        _has_hedge = bool(_ha_tab2)
        if _has_hedge:
            _ret_h2 = returns[asset_names].copy()
            _prices_h2 = st.session_state.get("prices")
            for _ht2, _hd2 in _ha_tab2.items():
                if _ht2 in _ret_h2.columns:
                    _ret_h2 = apply_hedge_to_returns(
                        _ret_h2, _ht2, _hd2["strike"], _hd2["spot"], _hd2["premium"]
                    )
            m_h = portfolio_metrics(
                np.array(w, dtype=float), _ret_h2[asset_names],
                rf, expected_return=saved.get("expected_return", 0),
                risk_geo=saved.get("risk_geo", 0),
                risk_mon=saved.get("risk_mon", 0),
                risk_sys=saved.get("risk_sys", 0),
            )
            _hedged_tickers_str = "، ".join(_ha_tab2.keys())
            st.markdown(
                f'<div class="bp-section"><span class="bp-section-text">'
                f'🛡️ پرتفو با هج فعال — {_hedged_tickers_str}'
                f'</span></div>',
                unsafe_allow_html=True
            )
            st.caption("متریک‌های زیر با احتساب Protective Put محاسبه شده‌اند. دلتا نسبت به پرتفوی بدون هج است.")
        else:
            m_h = None

        st.markdown('<div class="bp-section"><span class="bp-section-text">Risk &amp; Return Metrics</span></div>', unsafe_allow_html=True)
        c1,c2,c3,c4 = st.columns(4)
        # اگر هج فعال است، مقدار اصلی را به عنوان delta نشان می‌دهیم
        with c1:
            if m_h:
                _dret = (m_h["بازده سالانه"] - m["بازده سالانه"]) * 100
                st.metric("بازده سالانه (هج‌شده)", f"{m_h['بازده سالانه']*100:.2f}%",
                          delta=f"{_dret:+.2f}% vs بدون هج")
                _dsh = m_h["نسبت شارپ"] - m["نسبت شارپ"]
                st.metric("نسبت شارپ", f"{m_h['نسبت شارپ']:.3f}",
                          delta=f"{_dsh:+.3f}")
            else:
        ...
        with c2:
            if m_h:
                adj_ret_h = m_h["بازده تعدیل‌شده ریسک"]
                raw_ret_h = m_h["بازده سالانه"]
                _dadj = (adj_ret_h - m["بازده تعدیل‌شده ریسک"]) * 100
                delta_adj = f"{_dadj:+.2f}% vs بدون هج"
                st.metric("بازده تعدیل‌شده ریسک", f"{adj_ret_h*100:.2f}%", delta=delta_adj)
                _dcal = m_h["نسبت کالمار"] - m["نسبت کالمار"]
                st.metric("نسبت کالمار", f"{m_h['نسبت کالمار']:.3f}", delta=f"{_dcal:+.3f}")
            else:
        ...
        with c3:
            if m_h:
                _dvol = (m_h["نوسان سالانه"] - m["نوسان سالانه"]) * 100
                st.metric("نوسان سالانه", f"{m_h['نوسان سالانه']*100:.2f}%",
                          delta=f"{_dvol:+.2f}% vs بدون هج", delta_color="inverse")
                _dcvar = (m_h["CVaR 95%"] - m["CVaR 95%"]) * 100
                st.metric("CVaR 95%", f"{m_h['CVaR 95%']*100:.2f}%",
                          delta=f"{_dcvar:+.2f}% vs بدون هج", delta_color="inverse")
            else:
                st.metric("نوسان سالانه", f"{m['نوسان سالانه']*100:.2f}%")
                st.metric("CVaR 95%", f"{m['CVaR 95%']*100:.2f}%")
        with c4:
            if m_h:
                _dmdd = (m_h["حداکثر افت (Max Drawdown)"] - m["حداکثر افت (Max Drawdown)"]) * 100
                rec_h = m_h["ریکاوری تایم (روز)"]
                rec_months_h = int(rec_h/21)
                rec_str_h = f"{rec_months_h} ماه" if rec_months_h else f"{rec_h} روز"
                st.metric("ریکاوری تایم", rec_str_h)
                st.metric("حداکثر افت (MDD)", f"{m_h['حداکثر افت (Max Drawdown)']*100:.2f}%",
                          delta=f"{_dmdd:+.2f}% vs بدون هج", delta_color="inverse")
            else:
        ...
        # نوار اطلاعات بدون هج (وقتی هج فعال است)
        if _has_hedge:
            st.caption(
                f"📊 بدون هج — بازده: {m['بازده سالانه']*100:.2f}% | "
                f"نوسان: {m['نوسان سالانه']*100:.2f}% | "
                f"شارپ: {m['نسبت شارپ']:.3f} | "
                f"CVaR: {m['CVaR 95%']*100:.2f}% | "
                f"MDD: {m['حداکثر افت (Max Drawdown)']*100:.2f}%"
            )
        ...
        _w_arr_dd = np.array(w, dtype=float)
        port_ret_arr = returns[asset_names].values @ _w_arr_dd
        cum = (1+port_ret_arr).cumprod()
        roll_max = pd.Series(cum).cummax()
        dd = (pd.Series(cum)-roll_max)/roll_max
        fig_dd = go.Figure()
        # اگر هج فعال است، هر دو خط را نشان می‌دهیم
        if _has_hedge:
            _port_h_arr = _ret_h2[asset_names].values @ _w_arr_dd
            _cum_h = (1 + _port_h_arr).cumprod()
            _roll_max_h = pd.Series(_cum_h).cummax()
            _dd_h = (pd.Series(_cum_h) - _roll_max_h) / _roll_max_h
            fig_dd.add_trace(go.Scatter(
                x=prices.index[-len(_dd_h):], y=_dd_h.values*100,
                mode="lines", name="هج‌شده",
                line=dict(color="#5b9bd5", width=2),
                fill="tozeroy", fillcolor="rgba(91,155,213,0.10)"
            ))
            fig_dd.add_trace(go.Scatter(
                x=prices.index[-len(dd):], y=dd.values*100,
                mode="lines", name="اصلی",
...
with tab5:
    ...
    if opt_tab == "Protective Put":
        ...
        # ── انتخاب دارایی از پرتفو (اگر پرتفو بارگذاری شده) ──
        _prices_pp = st.session_state.get("prices")
        _portfolio_tickers = list(_prices_pp.columns) if _prices_pp is not None else []
        _w_pp = st.session_state.get("weights")

        if _portfolio_tickers:
            st.markdown("**🎯 انتخاب دارایی از پرتفو**")
            _col_sel, _col_info = st.columns([2, 1])
            with _col_sel:
                pp_asset = st.selectbox(
                    "دارایی برای بیمه‌کردن",
                    ["— وارد دستی —"] + _portfolio_tickers,
                    key="pp_asset_sel",
                    help="دارایی را از پرتفوی فعلی انتخاب کنید تا قیمت و وزن آن به‌صورت خودکار بارگذاری شود"
                )
            # auto-fill
            if pp_asset != "— وارد دستی —":
                _last_price = float(_prices_pp[pp_asset].iloc[-1])
                _asset_weight = float(_w_pp[list(_prices_pp.columns).index(pp_asset)]) if _w_pp is not None else None
                with _col_info:
                    st.metric("قیمت آخر", f"${_last_price:,.2f}")
                    if _asset_weight is not None:
                        st.metric("وزن در پرتفو", f"{_asset_weight*100:.1f}%")
            else:
                _last_price = None
                _asset_weight = None
            st.markdown("---")
        ...
        # ── ورودی‌های محاسبه ──
        _default_spot   = round(_last_price, 2) if _last_price else 100.0
        _default_strike = round(_last_price * 0.95, 2) if _last_price else 95.0

        co1, co2, co3 = st.columns(3)
        with co1:
            pp_spot    = st.number_input("قیمت فعلی دارایی ($)", value=_default_spot, step=1.0, key="pp_spot")
            pp_strike  = st.number_input("Strike Put ($)", value=_default_strike, step=1.0, key="pp_strike",
        ...
        if st.button("محاسبه Protective Put", use_container_width=True, key="pp_calc"):
            ...
            # ذخیره اطلاعات دارایی انتخاب‌شده برای اعمال هج
            st.session_state["pp_asset_info"] = {
                "ticker": pp_asset if pp_asset != "— وارد دستی —" else None,
                "spot": pp_spot,
                "strike": pp_strike,
                "premium": pp["premium"],
                "weight": _asset_weight,
                "shares": pp_shares,
                "days": pp_days,
            }
        ...
        if pp_r:
            ...
            # ── اعمال هج به پرتفو ──────────────────────────────────────────
            _pp_info = st.session_state.get("pp_asset_info", {})
            _pp_ticker = _pp_info.get("ticker")
            _prices_for_hedge = st.session_state.get("prices")
            _w_for_hedge = st.session_state.get("weights")

            if _pp_ticker and _prices_for_hedge is not None and _w_for_hedge is not None:
                st.markdown('<div class="bp-section"><span class="bp-section-text">تأثیر هج روی پرتفو</span></div>', unsafe_allow_html=True)

                _apply_col, _remove_col = st.columns(2)
                _current_hedged = st.session_state.get("hedged_assets", {})
                _already_hedged = _pp_ticker in _current_hedged

                with _apply_col:
                    if st.button(
                        f"✅ اعمال بیمه {_pp_ticker} به پرتفو",
                        use_container_width=True,
                        key="pp_apply_hedge",
                        type="primary" if not _already_hedged else "secondary"
                    ):
                        if "hedged_assets" not in st.session_state:
                            st.session_state["hedged_assets"] = {}
                        st.session_state["hedged_assets"][_pp_ticker] = {
                            "strike": _pp_info["strike"],
                            "spot": _pp_info["spot"],
                            "premium": _pp_info["premium"],
                            "days": _pp_info["days"],
                        }
                        st.success(f"✅ {_pp_ticker} در پرتفو بیمه شد!")
                        st.rerun()

                with _remove_col:
                    if _already_hedged:
                        if st.button(f"🗑️ حذف بیمه {_pp_ticker}", use_container_width=True, key="pp_remove_hedge"):
                            del st.session_state["hedged_assets"][_pp_ticker]
                            st.info(f"{_pp_ticker} از هج خارج شد.")
                            st.rerun()

                # نمایش مقایسه ریسک/بازده
                _hedged_assets_map = st.session_state.get("hedged_assets", {})
                if _hedged_assets_map:
                    _rets_orig  = _prices_for_hedge.pct_change().dropna()
                    _asset_names_h = list(_prices_for_hedge.columns)

                    # بازده هج‌شده
                    _rets_hedged = _rets_orig.copy()
                    for _ht, _hdata in _hedged_assets_map.items():
                        if _ht in _rets_hedged.columns:
                            _rets_hedged = apply_hedge_to_returns(
                                _rets_hedged, _ht,
                                _hdata["strike"], _hdata["spot"], _hdata["premium"]
                            )

                    _w_arr = np.array(_w_for_hedge, dtype=float)
                    _port_orig   = _rets_orig[_asset_names_h].values @ _w_arr
                    _port_hedged = _rets_hedged[_asset_names_h].values @ _w_arr

                    _vol_orig   = float(np.std(_port_orig)   * np.sqrt(252)) * 100
                    _vol_hedged = float(np.std(_port_hedged) * np.sqrt(252)) * 100
                    _ret_orig   = float((1 + _port_orig).prod() ** (252/len(_port_orig)) - 1) * 100
                    _ret_hedged = float((1 + _port_hedged).prod() ** (252/len(_port_hedged)) - 1) * 100
                    _sh_orig    = (_ret_orig/100 - rf) / (_vol_orig/100 + 1e-9)
                    _sh_hedged  = (_ret_hedged/100 - rf) / (_vol_hedged/100 + 1e-9)

                    # CVaR
                    _cvar_orig   = float(-np.percentile(_port_orig,   5)) * 100
                    _cvar_hedged = float(-np.percentile(_port_hedged, 5)) * 100

                    _hc1, _hc2, _hc3, _hc4 = st.columns(4)
                    with _hc1:
                        _dv = _vol_hedged - _vol_orig
                        st.metric("نوسان سالانه", f"{_vol_hedged:.2f}%",
                                  delta=f"{_dv:+.2f}%",
                                  delta_color="inverse")
                        st.caption("بدون هج: {:.2f}%".format(_vol_orig))
                    with _hc2:
                        _dr = _ret_hedged - _ret_orig
                        st.metric("بازده سالانه", f"{_ret_hedged:.2f}%",
                                  delta=f"{_dr:+.2f}%")
                        st.caption("بدون هج: {:.2f}%".format(_ret_orig))
                    with _hc3:
                        _ds = _sh_hedged - _sh_orig
                        st.metric("شارپ", f"{_sh_hedged:.3f}",
                                  delta=f"{_ds:+.3f}")
                        st.caption("بدون هج: {:.3f}".format(_sh_orig))
                    with _hc4:
                        _dc = _cvar_hedged - _cvar_orig
                        st.metric("CVaR 95%", f"{_cvar_hedged:.2f}%",
                                  delta=f"{_dc:+.2f}%",
                                  delta_color="inverse")
                        st.caption("بدون هج: {:.2f}%".format(_cvar_orig))

                    # نمودار مقایسه
                    _cum_orig   = pd.Series((1 + _port_orig).cumprod(),
                                            index=_rets_orig.index)
                    _cum_hedged = pd.Series((1 + _port_hedged).cumprod(),
                                            index=_rets_orig.index)
                    _fig_hcmp = go.Figure()
                    _fig_hcmp.add_trace(go.Scatter(
                        x=_cum_orig.index, y=_cum_hedged.values,
                        mode="lines", name="پرتفو هج‌شده",
                        line=dict(color=blue_t, width=2.5)
                    ))
                    _fig_hcmp.add_trace(go.Scatter(
                        x=_cum_orig.index, y=_cum_orig.values,
                        mode="lines", name="پرتفو اصلی",
                        line=dict(color="#888888", width=1.5, dash="dash")
                    ))
                    _fig_hcmp.update_layout(**get_plot_layout(
                        "PORTFOLIO — با هج vs بدون هج (ارزش ۱ دلار سرمایه‌گذاری)",
                        "تاریخ", "ارزش", 360
                    ))
                    st.plotly_chart(_fig_hcmp, use_container_width=True)

                    # لیست دارایی‌های هج‌شده
                    st.markdown("**📋 دارایی‌های بیمه‌شده در پرتفو:**")
                    for _ht2, _hd2 in _hedged_assets_map.items():
                        _w_idx = _asset_names_h.index(_ht2) if _ht2 in _asset_names_h else -1
                        _w_pct = _w_for_hedge[_w_idx]*100 if _w_idx >= 0 else 0
                        _floor = ((_hd2["strike"]/_hd2["spot"])-1)*100
                        st.markdown(
                            f"🛡️ **{_ht2}** — وزن: {_w_pct:.1f}% | "
                            f"Strike: ${_hd2['strike']:.2f} | "
                            f"کف بازده: {_floor:.1f}% | "
                            f"پرمیوم: ${_hd2['premium']:.3f}"
                        )
            elif _pp_info.get("ticker") is None and _prices_for_hedge is not None:
                st.info("💡 برای اعمال هج به پرتفو، دارایی را از لیست پرتفو انتخاب کنید (نه وارد دستی).")
...
with tab_bourse:
    ...
    def bc_metric_card(col, title, value, subtitle, color, note=""):
        note_html = ""
        if note:
            note_html = f"<div style='font-size:0.58rem;color:{muted_bc};margin-top:0.15rem;border-top:1px solid {border_bc};padding-top:0.2rem'>{note}</div>"
        div_style = metric_style.format(c=color)
        html = (
            f"<div style=\"{div_style}\">"
            f"<div style=\"font-size:0.58rem;font-weight:700;letter-spacing:0.12em;"
            f"color:{muted_bc};text-transform:uppercase;margin-bottom:0.3rem\">{title}</div>"
            f"<div style=\"font-family:'JetBrains Mono',monospace;font-size:1.15rem;"
            f"font-weight:700;color:{color}\">{value}</div>"
            f"<div style=\"font-size:0.6rem;color:{muted_bc};margin-top:0.25rem\">{subtitle}</div>"
            f"{note_html}"
            f"</div>"
        )
    ...
    with st.expander("جدول کامل Greeks و محاسبات", expanded=False):
        ...
        def _bc_info_row(lbl, val, color, border):
            return (
                f"<div style='display:flex;justify-content:space-between;padding:0.22rem 0;"
                f"border-bottom:1px solid {border}'>"
                f"<span style='font-size:0.68rem;color:#888;font-weight:600'>{lbl}</span>"
        ...
        with greeks_col1:
            ...
            rows_html = "".join(_bc_info_row(l, v, c, border_bc) for l, v, c in greeks_data)
            st.markdown(
                f"<div style='background:{card_bc};border:1px solid {border_bc};"
                f"border-radius:4px;padding:0.8rem 1rem'>"
                f"<div style='font-size:0.6rem;font-weight:700;letter-spacing:0.12em;color:{muted_bc};"
                f"text-transform:uppercase;margin-bottom:0.6rem'>Greeks کامل</div>"
                f"{rows_html}</div>",
                unsafe_allow_html=True
            )

        with greeks_col2:
            ul_type_label = "Call اختیار خرید" if bc_is_call else "Put اختیار فروش"
            ul_type_color = green_bc if bc_is_call else red_bc
            contract_data = [
                ("دارایی پایه", ul['label'], blue_bc),
                ("نوع", ul_type_label, ul_type_color),
            ...
            rows_html2 = "".join(_bc_info_row(l, v, c, border_bc) for l, v, c in contract_data)
            st.markdown(
                f"<div style='background:{card_bc};border:1px solid {border_bc};"
                f"border-radius:4px;padding:0.8rem 1rem'>"
                f"<div style='font-size:0.6rem;font-weight:700;letter-spacing:0.12em;color:{muted_bc};"
                f"text-transform:uppercase;margin-bottom:0.6rem'>مشخصات قرارداد</div>"
                f"{rows_html2}</div>",
                unsafe_allow_html=True
            )
    ...
    with strat_col1:
        strat_rows_html = "".join(
            f"<div style='display:flex;justify-content:space-between;padding:0.25rem 0;"
            f"border-bottom:1px solid {border_bc}'>"
            f"<span style='font-size:0.68rem;color:{muted_bc};font-weight:600'>{lbl}</span>"
            f"<span style='font-family:JetBrains Mono,monospace;font-size:0.75rem;font-weight:700;color:{color}'>{val}</span>"
            f"</div>"
            for lbl, val, color in strat_info
        )
        st.markdown(
            f"<div style='background:{card_bc};border:1px solid {border_bc};"
            f"border-radius:4px;padding:0.9rem 1rem'>"
            f"<div style='font-size:0.6rem;font-weight:700;letter-spacing:0.14em;color:{muted_bc};"
            f"text-transform:uppercase;margin-bottom:0.6rem'>{strategy_bc}</div>"
            f"{strat_rows_html}</div>",
            unsafe_allow_html=True
        )
    with strat_col2:
    ...
    chk_rows_html = ""
    for ok, txt in checks_bc:
        icon = "✓" if ok else "✗"
        row_color = green_bc if ok else red_bc
        txt_color = "#909090" if is_dark else "#444440" if ok else "#888882"
        chk_rows_html += (
            f"<div style='display:flex;align-items:flex-start;gap:8px;padding:0.28rem 0;"
            f"border-bottom:1px solid {border_bc}'>"
            f"<span style='color:{row_color};font-size:0.85rem;min-width:16px;margin-top:1px'>{icon}</span>"
            f"<span style='font-size:0.72rem;color:{txt_color};line-height:1.5'>{txt}</span>"
            f"</div>"
        )
    st.markdown(
        f"<div style='background:{card_bc};border:1px solid {border_bc};border-radius:4px;padding:0.9rem 1.1rem'>"
        f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:0.7rem'>"
        f"<span style='font-size:0.62rem;font-weight:700;letter-spacing:0.12em;"
        f"color:{muted_bc};text-transform:uppercase'>آمادگی معامله</span>"
        f"<span style='font-family:JetBrains Mono,monospace;font-weight:700;"
        f"font-size:0.9rem;color:{chk_color}'>{passed_bc}/{len(checks_bc)} ✓</span>"
        f"</div>"
        f"{chk_rows_html}"
        f"<div style='margin-top:0.6rem;padding-top:0.4rem;border-top:1px solid {border_bc};"
        f"font-size:0.62rem;color:{muted_bc};line-height:1.7'>"
        f"💡 <strong>راهنما:</strong> اختیار ایده‌آل: OTM · IV ۲۰–۶۰٪ · Delta ۰.۲–۰.۴ · DTE ۲۱–۴۵ روز · R/R بالای ۲<br>"
        f"📌 بورس کالای ایران: هر قرارداد = {lot:,} واحد · سررسید ماهانه · اعمال اروپایی"
        f"</div></div>",
        unsafe_allow_html=True
    )
...
with tab_ime:
    ...
    def ime_fmt(v, dec=0):
        try:
            return f"{int(v):,}" if dec == 0 else f"{float(v):,.{dec}f}"
        except Exception:
            return str(v)
    ...
    def _row_html(lbl, val, color, border, min_w="110px"):
        return (
            f"<div style='display:flex;justify-content:space-between;align-items:flex-start;"
            f"padding:0.22rem 0;border-bottom:1px solid {border}'>"
            f"<span style='font-size:0.63rem;color:{muted_im};font-weight:600;min-width:{min_w}'>{lbl}</span>"
            f"<span style='font-family:JetBrains Mono,monospace;font-size:0.65rem;"
            f"font-weight:600;color:{color};text-align:right;flex:1'>{val}</span>"
            f"</div>"
        )

    # ── API layer ──────────────────────────────────────────────────
    @st.cache_data(show_spinner=False, ttl=60)
    def ime_fetch(endpoint: str, params: dict | None = None) -> dict:
        try:
            ...
            with _ureq.urlopen(req, timeout=10) as resp:
                return _json_ime.loads(resp.read().decode("utf-8"))
        except Exception as e:
            return {"_error": str(e)}

    @st.cache_data(show_spinner=False, ttl=300)
    def ime_fetch_history(contract_code: str, from_date: str, to_date: str) -> dict:
        return ime_fetch(
            f"/commodity-deposit-certificate/{contract_code}/history",
            params={"from": from_date, "to": to_date}
        )

    def ime_parse_rows(data: dict) -> list:
    ...
    DEMO_ROWS = [
        {"commodity":"شمش طلا","contract_code":"CD1GOB0001",
         ...
         "contract_size":10,"contract_size_unit":"گرم","contract_currency":"ریال",
         ...
         "date_order":"1404-07-02","time_order":"16:43:27",
         "pd1":13239780,"pd2":13239770,"pd3":13230040,
         "qd1":558699,"qd2":32,"qd3":203,
         "po1":0,"po2":0,"po3":0,"qo1":0,"qo2":0,"qo3":0,
         "date_update":"1404-07-02","time_update":"16:43:56","time":"16:43:51"},
        {"commodity":"شمش نقره","contract_code":"CD1SIB0001",
         "contract_description":"گواهی سپرده پیوسته شمش نقره 999.9",
         "contract_size":500,"contract_size_unit":"گرم","contract_currency":"ریال",
         "date_y":"1404-07-01","py":376500,
         "pf_time":"12:00:00","pf":389200,"pfc":12700,"pfp":3.37,
        ...
        {"commodity":"مس کاتد","contract_code":"CD1CUB0001",
         ...
         "date_order":"1404-07-02","time_order":"16:38:00",
        ...
        {"commodity":"شمش روی","contract_code":"CD1ZNB0001",
         ...
         "date_order":"1404-07-02","time_order":"16:35:00",
         "pd1":479100,"pd2":479000,"pd3":478500,
         "qd1":1000,"qd2":500,"qd3":200,
         "po1":480000,"po2":481000,"po3":482000,
         "qo1":500,"qo2":300,"qo3":200,
         "date_update":"1404-07-02","time_update":"16:43:56","time":"16:35:00"},
        {"commodity":"میلگرد","contract_code":"CD1RBB0001",
         "contract_description":"گواهی سپرده میلگرد",
         "contract_size":5000,"contract_size_unit":"کیلوگرم","contract_currency":"ریال",
         "date_y":"1404-07-01","py":578000,
         "pf_time":"12:00:00","pf":580000,"pfc":2000,"pfp":0.35,
         "pmax":583000,"pmaxc":5000,"pmaxp":0.86,
         "pmin":577000,"pminc":-1000,"pminp":-0.17,
         "pl_time":"16:30:00","pl":581000,"plc":3000,"plp":0.52,
         "tno":75,"tvol":15000,"tval":8715000,"tval_unit":"هزار ریال",
         "date_order":"1404-07-02","time_order":"16:30:00",
         "pd1":581000,"pd2":580000,"pd3":579500,
         "qd1":3000,"qd2":500,"qd3":200,
         "po1":582000,"po2":583000,"po3":0,
         "qo1":1000,"qo2":500,"qo3":0,
         "date_update":"1404-07-02","time_update":"16:43:56","time":"16:30:00"},
    ]

    # ── header ────────────────────────────────────────────────────
    st.markdown(
        f"<div style='display:flex;align-items:center;justify-content:space-between;"
        f"margin-bottom:1rem;padding-bottom:0.8rem;border-bottom:2px solid var(--border2)'>"
        f"<div style='display:flex;align-items:center;gap:12px'>"
        f"<span style='font-size:1.6rem'>📡</span>"
        f"<div>"
        f"<div style='font-family:JetBrains Mono,monospace;font-size:0.88rem;"
        f"font-weight:700;letter-spacing:0.1em;color:var(--accent)'>IME LIVE — گواهی سپرده کالایی</div>"
        f"<div style='font-size:0.62rem;color:{muted_im};letter-spacing:0.08em;margin-top:2px'>"
        f"بورس کالای ایران · API رسمی · کلید: {IME_API_KEY[:8]}…</div>"
        f"</div></div>"
        f"<div style='font-family:JetBrains Mono,monospace;font-size:0.6rem;"
        f"color:{muted_im};text-align:right;line-height:1.8'>نرخ تازه‌سازی: ۶۰ ثانیه<br>منبع: api.ime.co.ir</div>"
        f"</div>",
        unsafe_allow_html=True
    )
    ...
    with ctrl1:
        st.caption("endpoint: `/commodity-deposit-certificate/last-changes` — شامل تمام فیلدهای API")
    with ctrl2:
        auto_refresh = st.checkbox("بروزرسانی خودکار", value=False, key="ime_auto")
    with ctrl3:
        if st.button("🔄 بروزرسانی", use_container_width=True, key="ime_refresh"):
            st.cache_data.clear()
    ...
    rows_all = ime_parse_rows(raw_all)
    use_demo = False

    if raw_all and "_error" in raw_all:
        err_msg = raw_all["_error"]
        st.markdown(
            f"<div style='background:{card_im};border:1px solid {red_im};"
            f"border-left:4px solid {red_im};border-radius:4px;padding:1rem 1.2rem;margin-bottom:1rem'>"
            f"<div style='font-family:JetBrains Mono,monospace;font-size:0.75rem;"
            f"font-weight:700;color:{red_im};margin-bottom:0.4rem'>❌ خطا در اتصال به API</div>"
            f"<div style='font-size:0.72rem;color:{muted_im};line-height:1.9'>{err_msg}<br><br>"
            f"<strong>راه‌حل‌های احتمالی:</strong><br>"
            f"① در صورت استفاده از Streamlit Cloud، دامنه <code>api.ime.co.ir</code> را در egress settings اضافه کنید.<br>"
            f"② برنامه را روی سرور ایرانی (VPS) اجرا کنید.<br>"
            f"③ برای تست رابط کاربری، داده نمونه زیر را فعال کنید.</div></div>",
            unsafe_allow_html=True
        )
        use_demo = st.checkbox("استفاده از داده نمونه (DEMO MODE)", value=True, key="ime_demo")
        if use_demo:
            rows_all = DEMO_ROWS
            st.info("📊 داده نمونه برای نمایش رابط کاربری استفاده می‌شود. حالت Demo")

    # ─────────────────────────────────────────────────────────────
    if rows_all:
        last_upd = rows_all[0].get("date_update","—") + "  " + rows_all[0].get("time_update","")
        last_chg = rows_all[0].get("time","—")

        if use_demo:
            badge = (f"<span style='font-size:0.6rem;padding:2px 10px;"
                     f"background:rgba(200,168,75,0.15);border:1px solid {gold_im};"
                     f"border-radius:10px;color:{gold_im};font-family:monospace'>◉ DEMO</span>")
        else:
            badge = (f"<span style='font-size:0.6rem;padding:2px 10px;"
                     f"background:rgba(90,170,120,0.15);border:1px solid {green_im};"
                     f"border-radius:10px;color:{green_im};font-family:monospace'>● LIVE</span>")

        st.markdown(
            f"<div style='display:flex;align-items:center;gap:14px;margin-bottom:0.8rem;"
            f"padding:0.5rem 0.9rem;background:{card_im};border:1px solid {border_im};"
            f"border-radius:4px'>"
            f"<span>🕐</span>"
            f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;color:{muted_im}'>"
            f"آخرین داده: <strong style='color:var(--accent)'>{last_upd}</strong></span>"
            f"<span style='font-size:0.6rem;color:{muted_im}'>|</span>"
            f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;color:{muted_im}'>"
            f"آخرین تغییر: <strong style='color:var(--accent)'>{last_chg}</strong></span>"
            f"<span style='font-size:0.6rem;color:{muted_im}'>|</span>"
            f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;color:{muted_im}'>"
            f"{len(rows_all)} نماد</span>"
            f"{badge}</div>",
            unsafe_allow_html=True
        )

        # ══ ① کارت‌های خلاصه ══════════════════════════════════
        st.markdown('<div class="bp-section"><span class="bp-section-text">① تابلوی زنده — آخرین قیمت‌ها</span></div>', unsafe_allow_html=True)

        n_cols    = min(len(rows_all), 3)
        card_cols = st.columns(n_cols)

        for idx_c, row in enumerate(rows_all):
            commodity = row.get("commodity","—")
            pl_v   = safe_float(row.get("pl", 0))
            plc_v  = safe_float(row.get("plc", 0))
            ...
            tval_v = safe_float(row.get("tval",0))
            tval_u = row.get("tval_unit","هزار ریال")
            pl_time= row.get("pl_time","—")
            qd1_v  = safe_float(row.get("qd1",0))
            qo1_v  = safe_float(row.get("qo1",0))
            size_v = row.get("contract_size","—")
            size_u = row.get("contract_size_unit","")
            clr    = green_im if plc_v >= 0 else red_im
            sign   = "▲" if plc_v >= 0 else "▼"

            # صف خرید / فروش
            if qd1_v > 0 and qo1_v == 0:
                queue_html = f"<span style='font-size:0.58rem;color:{green_im};font-weight:700'>💚 صف خرید</span>"
            elif qo1_v > 0 and qd1_v == 0:
                queue_html = f"<span style='font-size:0.58rem;color:{red_im};font-weight:700'>🔴 صف فروش</span>"
            elif qd1_v > 0 and qo1_v > 0:
                ratio = qd1_v / (qo1_v + 1e-9)
                queue_html = (f"<span style='font-size:0.58rem;color:{muted_im}'>"
                              f"خرید/فروش: {ratio:.1f}x</span>")
            else:
                queue_html = ""

            with card_cols[idx_c % n_cols]:
                st.markdown(
                    f"<div style='background:{card_im};border:1px solid {border_im};"
                    f"border-top:3px solid {clr};border-radius:4px;"
                    f"padding:0.8rem 0.9rem;margin-bottom:0.6rem'>"
                    f"<div style='font-size:0.58rem;font-weight:700;letter-spacing:0.12em;"
                    f"color:{muted_im};text-transform:uppercase;margin-bottom:0.35rem'>{commodity}</div>"
                    f"<div style='font-family:JetBrains Mono,monospace;font-size:1.2rem;"
                    f"font-weight:700;color:var(--accent);line-height:1.1'>{ime_fmt(pl_v)}</div>"
                    f"<div style='font-size:0.6rem;color:{muted_im};margin-bottom:0.4rem'>ریال · {size_v} {size_u}</div>"
                    ...
                    f"font-weight:700;color:{clr}'>{sign} {ime_fmt(abs(plc_v))}</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                    f"font-weight:600;color:{clr}'>({ime_delta_sign(plp_v)}%)</span>"
                    f"</div>"
                    f"{queue_html}"
                    f"<div style='border-top:1px solid {border_im};padding-top:0.35rem;margin-top:0.35rem'>"
                    f"<div style='display:flex;justify-content:space-between'>"
                    f"<span style='font-size:0.6rem;color:{muted_im}'>دیروز</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.62rem;"
                    f"color:var(--accent)'>{ime_fmt(py_v)}</span></div>"
                    f"<div style='display:flex;justify-content:space-between'>"
                    f"<span style='font-size:0.6rem;color:{muted_im}'>بالا / پایین</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.6rem;"
                    f"color:{muted_im}'>{ime_fmt(pmax_v)} / {ime_fmt(pmin_v)}</span></div>"
                    f"<div style='display:flex;justify-content:space-between'>"
                    f"<span style='font-size:0.6rem;color:{muted_im}'>تعداد / حجم</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.6rem;"
                    f"color:{muted_im}'>{ime_fmt(tno_v)} / {ime_fmt(tvol_v)}</span></div>"
                    f"<div style='display:flex;justify-content:space-between'>"
                    f"<span style='font-size:0.6rem;color:{muted_im}'>ارزش معاملات</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.6rem;"
                    f"color:{gold_im}'>{ime_fmt(tval_v)} {tval_u}</span></div>"
                    f"<div style='display:flex;justify-content:space-between'>"
                    f"<span style='font-size:0.6rem;color:{muted_im}'>آخرین قیمت</span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.6rem;"
                    f"color:{muted_im}'>{pl_time}</span></div>"
                    f"</div></div>",
                    unsafe_allow_html=True
                )

        # ══ جدول کامل همه فیلدها ════════════════════════════════
        ...
        for row in rows_all:
            plc_v  = safe_float(row.get("plc", 0))
            plp_v  = safe_float(row.get("plp", 0))
            pfp_v  = safe_float(row.get("pfp", 0))
            pmaxp_v= safe_float(row.get("pmaxp",0))
            pminp_v= safe_float(row.get("pminp",0))
            tval_v = safe_float(row.get("tval", 0))
            tval_u = row.get("tval_unit","هزار ریال")
            table_rows.append({
                ...
                "اولین %":        f"{'+' if pfp_v>=0 else ''}{pfp_v:.2f}%",
                "بالاترین":       f"{ime_fmt(row.get('pmax',0))} ({pmaxp_v:+.2f}%)",
                "پایین‌ترین":     f"{ime_fmt(row.get('pmin',0))} ({pminp_v:+.2f}%)",
                "قیمت دیروز":     ime_fmt(row.get("py",0)),
                "تعداد معاملات":  ime_fmt(row.get("tno",0)),
                "حجم":            ime_fmt(row.get("tvol",0)),
                f"ارزش ({tval_u})": ime_fmt(tval_v),
                "بهترین خرید":   ime_fmt(row.get("pd1",0)),
                "حجم خرید":       ime_fmt(row.get("qd1",0)),
                "بهترین فروش":   ime_fmt(row.get("po1",0)) if safe_float(row.get("po1",0)) else "—",
                "حجم فروش":       ime_fmt(row.get("qo1",0)) if safe_float(row.get("qo1",0)) else "—",
                "زمان":           row.get("pl_time","—"),
                "زمان سفارش":    row.get("time_order","—"),
            })
        st.dataframe(pd.DataFrame(table_rows), use_container_width=True, hide_index=True)

        # ══ نمودارهای اصلی ════════════════════════════════════════
        st.markdown('<div class="bp-section"><span class="bp-section-text">نمودار تغییر درصدی</span></div>', unsafe_allow_html=True)

        names_c  = [r.get("commodity","?") for r in rows_all]
        change_c = [safe_float(r.get("plp",0)) for r in rows_all]
        clrs_c   = [green_im if v >= 0 else red_im for v in change_c]

        fig_bar = go.Figure()
        fig_bar.add_trace(go.Bar(
            x=names_c, y=change_c,
            marker_color=clrs_c,
            marker_line=dict(color="rgba(0,0,0,0.15)", width=0.5),
            text=[f"{v:+.2f}%" for v in change_c],
            textposition="outside",
            textfont=dict(size=10, family="JetBrains Mono"),
            hovertemplate="%{x}<br>تغییر: %{y:+.2f}%<extra></extra>",
        ))
        fig_bar.add_hline(y=0, line_color="rgba(128,128,128,0.3)", line_width=1)
        fig_bar.update_layout(**get_plot_layout("تغییر درصدی گواهی‌های کالایی","","تغییر (%)",360))
        ...
        vol_c = [safe_float(r.get("tvol",0)) for r in rows_all]
        val_c = [safe_float(r.get("tval",0))/1e6 for r in rows_all]
        tno_c = [safe_float(r.get("tno",0)) for r in rows_all]

        fig_vol = go.Figure()
        fig_vol.add_trace(go.Bar(
            x=names_c, y=vol_c, name="حجم معاملات",
            marker_color=blue_im, opacity=0.8, yaxis="y",
            hovertemplate="%{x}<br>حجم: %{y:,.0f}<extra></extra>",
        ))
        fig_vol.add_trace(go.Scatter(
            x=names_c, y=val_c, name="ارزش (میلیون ریال)",
            mode="lines+markers",
        ...
        fig_vol.update_layout(**get_plot_layout("حجم و ارزش معاملات","","حجم (واحد)",380))
        fig_vol.update_layout(
            yaxis2=dict(title="ارزش (میلیون ریال)", overlaying="y", side="right",
        ...
        # ══ ② جزئیات نماد انتخابی ════════════════════════════════
        st.markdown('<div class="bp-section"><span class="bp-section-text">② جزئیات نماد — دفتر سفارشات و مشخصات کامل</span></div>', unsafe_allow_html=True)

        sym_opts = {r.get("contract_code","?"): r.get("commodity","?") for r in rows_all}
        sel_code = st.selectbox(
            "انتخاب نماد برای تحلیل جزئیات",
            list(sym_opts.keys()),
            format_func=lambda k: f"{sym_opts[k]}  ·  {k}",
            key="ime_sel_symbol"
        )
        ...
        if sel_row:
            ...
            with c1:
                ob_html = ""
                for _i in range(3):
                    _bg  = "background:rgba(91,155,213,0.08);" if _i == 0 else ""
                    _fw  = "700" if _i == 0 else "500"
                    _qd  = ime_fmt(sel_row.get(f"qd{_i+1}", 0))
                    _pdv = ime_fmt(sel_row.get(f"pd{_i+1}", 0))
                    _por = safe_float(sel_row.get(f"po{_i+1}", 0))
                    _qor = safe_float(sel_row.get(f"qo{_i+1}", 0))
                    _po  = ime_fmt(_por) if _por else "—"
                    _qo  = ime_fmt(_qor) if _qor else "—"
                    ob_html += (
                        f"<div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;"
                        f"gap:4px;padding:0.22rem 0;border-bottom:1px solid {border_im};{_bg}'>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                        f"font-weight:{_fw};color:{green_im};text-align:right'>{_qd}</span>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                        f"font-weight:{_fw};color:{green_im};text-align:right'>{_pdv}</span>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                        f"font-weight:{_fw};color:{red_im}'>{_po}</span>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                        f"font-weight:{_fw};color:{red_im}'>{_qo}</span>"
                        f"</div>"
                    )

                _bid1   = safe_float(sel_row.get("pd1",0))
                _ask1   = safe_float(sel_row.get("po1",0))
                _spread = abs(_ask1 - _bid1)
                _time_order = sel_row.get("time_order","—")
                _date_order = sel_row.get("date_order","—")

                st.markdown(
                    f"<div style='background:{card_im};border:1px solid {border_im};"
                    f"border-radius:4px;padding:0.9rem 1rem;margin-bottom:0.5rem'>"
                    f"<div style='font-size:0.6rem;font-weight:700;letter-spacing:0.15em;"
                    f"color:{muted_im};text-transform:uppercase;margin-bottom:0.5rem'>"
                    f"📋 دفتر سفارشات — {sel_row.get('commodity','—')}</div>"
                    f"<div style='font-size:0.58rem;color:{muted_im};margin-bottom:0.5rem'>"
                    f"آخرین تغییر: {_date_order} {_time_order}</div>"
                    f"<div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;"
                    f"gap:4px;padding:0.3rem 0;border-bottom:2px solid {border_im};margin-bottom:0.3rem'>"
                    f"<span style='font-size:0.6rem;font-weight:700;color:{green_im};text-align:right'>حجم خرید</span>"
                    f"<span style='font-size:0.6rem;font-weight:700;color:{green_im};text-align:right'>قیمت خرید</span>"
                    f"<span style='font-size:0.6rem;font-weight:700;color:{red_im}'>قیمت فروش</span>"
                    f"<span style='font-size:0.6rem;font-weight:700;color:{red_im}'>حجم فروش</span>"
                    f"</div>"
                    f"{ob_html}"
                    f"<div style='margin-top:0.6rem;padding:0.45rem 0.7rem;"
                    f"background:rgba(91,155,213,0.06);border-radius:3px;"
                    f"display:flex;justify-content:space-between;align-items:center'>"
                    f"<div>"
                    f"<div style='font-size:0.56rem;color:{muted_im}'>بهترین خرید</div>"
                    f"<div style='font-family:JetBrains Mono,monospace;font-size:0.8rem;"
                    f"font-weight:700;color:{green_im}'>{ime_fmt(_bid1)}</div>"
                    f"</div>"
                    f"<div style='text-align:center'>"
                    f"<div style='font-size:0.56rem;color:{muted_im}'>اسپرد</div>"
                    f"<div style='font-family:JetBrains Mono,monospace;font-size:0.8rem;"
                    f"font-weight:700;color:{gold_im}'>{ime_fmt(_spread)}</div>"
                    f"</div>"
                    f"<div style='text-align:right'>"
                    f"<div style='font-size:0.56rem;color:{muted_im}'>بهترین فروش</div>"
                    f"<div style='font-family:JetBrains Mono,monospace;font-size:0.8rem;"
                    f"font-weight:700;color:{red_im}'>{ime_fmt(_ask1) if _ask1 else '—'}</div>"
                    f"</div>"
                    f"</div></div>",
                    unsafe_allow_html=True
                )
            ...
            # ── مشخصات کامل قرارداد ──────────────────────────────
            with c2:
                _plp   = safe_float(sel_row.get("plp", 0))
                _pmaxp = safe_float(sel_row.get("pmaxp",0))
                _pminp = safe_float(sel_row.get("pminp",0))
                _pfp   = safe_float(sel_row.get("pfp",0))
                _pmaxc = safe_float(sel_row.get("pmaxc",0))
                _pminc = safe_float(sel_row.get("pminc",0))
                _tval  = safe_float(sel_row.get("tval",0))
                _tvu   = sel_row.get("tval_unit","هزار ریال")
                _pl2   = safe_float(sel_row.get("pl",1))
                _pmax2 = safe_float(sel_row.get("pmax",1))
                _pmin2 = safe_float(sel_row.get("pmin",1))
                _drng  = (_pmax2 - _pmin2) / _pl2 * 100 if _pl2 else 0

                details = [
                    ("کد قرارداد",        sel_row.get("contract_code","—"),    blue_im),
                    ("توضیح قرارداد",     sel_row.get("contract_description","—")[:40], muted_im),
                    ("اندازه قرارداد",    f"{sel_row.get('contract_size','—')} {sel_row.get('contract_size_unit','')}", gold_im),
                    ("ارز",               sel_row.get("contract_currency","ریال"), muted_im),
                    ("تاریخ معامله قبل",  sel_row.get("date_y","—"), muted_im),
                    ("قیمت پایانی دیروز",f"{ime_fmt(sel_row.get('py',0))} ریال", muted_im),
                    ("اولین قیمت",        f"{ime_fmt(sel_row.get('pf',0))} ریال ({ime_delta_sign(_pfp)}%)", blue_im),
                    ("زمان اولین قیمت",   sel_row.get("pf_time","—"), muted_im),
                    ("بالاترین",          f"{ime_fmt(_pmax2)} ریال ({ime_delta_sign(_pmaxp)}%) — Δ{ime_fmt(_pmaxc)}", green_im),
                    ("پایین‌ترین",        f"{ime_fmt(_pmin2)} ریال ({ime_delta_sign(_pminp)}%) — Δ{ime_fmt(_pminc)}", red_im),
                    ("دامنه روز",          f"{_drng:.2f}%", gold_im),
                    ("آخرین قیمت",        f"{ime_fmt(_pl2)} ریال ({ime_delta_sign(_plp)}%)", green_im if _plp>=0 else red_im),
                    ("زمان آخرین قیمت",   sel_row.get("pl_time","—"), muted_im),
                    ("تعداد معاملات",      ime_fmt(sel_row.get("tno",0)), blue_im),
                    ("حجم معاملات",       ime_fmt(sel_row.get("tvol",0)), blue_im),
                    (f"ارزش ({_tvu})",     ime_fmt(_tval), gold_im),
                    ("تاریخ داده",         sel_row.get("date_update","—"), muted_im),
                    ("زمان داده",          sel_row.get("time_update","—"), muted_im),
                ]

                det_html = "".join(_row_html(l, v, c, border_im) for l, v, c in details)
                st.markdown(
                    f"<div style='background:{card_im};border:1px solid {border_im};"
                    f"border-radius:4px;padding:0.9rem 1rem'>"
                    f"<div style='font-size:0.6rem;font-weight:700;letter-spacing:0.15em;"
                    f"color:{muted_im};text-transform:uppercase;margin-bottom:0.6rem'>"
                    f"📌 مشخصات کامل قرارداد</div>"
                    f"{det_html}</div>",
                    unsafe_allow_html=True
                )

            # ── نمودار Order Book ─────────────────────────────────
            st.markdown('<div class="bp-section"><span class="bp-section-text">نمودار دفتر سفارشات</span></div>', unsafe_allow_html=True)

            bid_prices = [safe_float(sel_row.get(f"pd{i}",0)) for i in range(1,4)]
            bid_qtys   = [safe_float(sel_row.get(f"qd{i}",0)) for i in range(1,4)]
            ask_prices = [safe_float(sel_row.get(f"po{i}",0)) for i in range(1,4)]
            ask_qtys   = [safe_float(sel_row.get(f"qo{i}",0)) for i in range(1,4)]
            ...
            if valid_bids or valid_asks:
                ...
                if valid_bids:
                    fig_ob.add_trace(go.Bar(
                        ...
                        y=[q for _,q in valid_bids],
                        name="خرید (Bid)", marker_color=green_im,
                        text=[ime_fmt(q) for _,q in valid_bids],
                        textposition="outside",
                        textfont=dict(size=9, family="JetBrains Mono"),
                    ))
                if valid_asks:
                    fig_ob.add_trace(go.Bar(
                        ...
                        y=[q for _,q in valid_asks],
                        name="فروش (Ask)", marker_color=red_im,
                        text=[ime_fmt(q) for _,q in valid_asks],
                        textposition="outside",
                        textfont=dict(size=9, family="JetBrains Mono"),
                    ))
                fig_ob.update_layout(**get_plot_layout(
                    f"Order Book — {sel_row.get('commodity','')}","","حجم سفارش",300))
                fig_ob.update_layout(barmode="group")
                st.plotly_chart(fig_ob, use_container_width=True)
            else:
                st.info("در حال حاضر سفارش فعالی در دفتر سفارشات ثبت نشده.")
        ...
        with hc1:
            hist_symbol = st.selectbox(
                "نماد تاریخچه", list(sym_opts.keys()),
                format_func=lambda k: f"{sym_opts[k]}  ·  {k}",
                key="ime_hist_sym"
            )
        with hc2:
        ...
        if st.button("دریافت تاریخچه", use_container_width=True, key="ime_hist_fetch"):
            ...
            st.session_state["ime_hist_sym_lbl"] = sym_opts.get(hist_symbol, hist_symbol)
        ...
        if raw_hist:
            if "_error" in raw_hist:
                ...
                if use_demo:
                    ...
                    for day in range(120):
                        ...
                        hist_rows.append({
                            "date":  f"1404-{(day//30)+1:02d}-{(day%30)+1:02d}",
                            "open":  int(base*0.99), "high": int(base*1.012),
                            "low":   int(base*0.988), "close": int(base),
            ...
            if hist_rows:
                opens  = [safe_float(r.get("open",  r.get("pf",  0))) for r in hist_rows]
                highs  = [safe_float(r.get("high",  r.get("pmax",0))) for r in hist_rows]
                lows   = [safe_float(r.get("low",   r.get("pmin",0))) for r in hist_rows]
                closes = [safe_float(r.get("close", r.get("pl",  0))) for r in hist_rows]
                dates  = [r.get("date", r.get("date_y", str(i))) for i,r in enumerate(hist_rows)]
                vols   = [safe_float(r.get("volume", r.get("tvol",0))) for r in hist_rows]

                fig_c = go.Figure()
                fig_c.add_trace(go.Candlestick(
                    x=dates, open=opens, high=highs, low=lows, close=closes,
                    name=hist_lbl,
                    increasing=dict(line=dict(color=green_im,width=1.5),fillcolor="rgba(90,170,120,0.7)"),
                    decreasing=dict(line=dict(color=red_im, width=1.5),fillcolor="rgba(204,85,85,0.7)"),
                ))
                if len(closes) >= 20:
                    ma20 = pd.Series(closes).rolling(20).mean().values
                    fig_c.add_trace(go.Scatter(x=dates, y=ma20, mode="lines", name="MA20",
                                               line=dict(color=blue_im, width=1.5, dash="dot")))
                if len(closes) >= 50:
                    ma50 = pd.Series(closes).rolling(50).mean().values
                    fig_c.add_trace(go.Scatter(x=dates, y=ma50, mode="lines", name="MA50",
                                               line=dict(color=gold_im, width=1.5, dash="dash")))

                fig_c.update_layout(**get_plot_layout(f"نمودار شمعی — {hist_lbl}","تاریخ","قیمت (ریال)",500))
                fig_c.update_layout(xaxis_rangeslider_visible=False, yaxis=dict(tickformat=",.0f"))
                st.plotly_chart(fig_c, use_container_width=True)

                vol_clrs = [blue_im] + [green_im if closes[i]>=closes[i-1] else red_im for i in range(1,len(closes))]
                fig_vh = go.Figure()
                fig_vh.add_trace(go.Bar(x=dates, y=vols, marker_color=vol_clrs,
                                        marker_line=dict(width=0), name="حجم", opacity=0.8,
                                        hovertemplate="%{x}<br>حجم: %{y:,.0f}<extra></extra>"))
                fig_vh.update_layout(**get_plot_layout("حجم معاملات تاریخی","تاریخ","حجم",200))
                st.plotly_chart(fig_vh, use_container_width=True)

                closes_arr = np.array([c for c in closes if c > 0])
                if len(closes_arr) > 1:
                    ret_h = np.diff(closes_arr) / closes_arr[:-1]
                    sa, sb, sc2, sd = st.columns(4)
                    with sa:
                        total_r = (closes_arr[-1]/closes_arr[0]-1)*100
                        st.metric("بازده کل دوره", f"{total_r:+.2f}%")
                        st.caption(f"{len(closes_arr)} روز معاملاتی")
                    with sb:
                        ann_v = float(np.std(ret_h)*np.sqrt(252)*100)
                        st.metric("نوسان سالانه‌شده", f"{ann_v:.2f}%")
                    with sc2:
                        mx_h = float(np.max(closes_arr))
                        mn_h = float(np.min(closes_arr))
                        st.metric("دامنه نوسان کل", f"{(mx_h-mn_h)/mx_h*100:.2f}%")
                        st.caption(f"بالا: {ime_fmt(mx_h)} | پایین: {ime_fmt(mn_h)}")
                    with sd:
                        pos_d = int(np.sum(ret_h>0))
                        neg_d = int(np.sum(ret_h<0))
                        st.metric("روزهای مثبت / منفی", f"{pos_d} / {neg_d}")
                        st.caption(f"نرخ موفقیت: {pos_d/(pos_d+neg_d)*100:.1f}%")

        # ══ ④ رادار مقایسه‌ای ════════════════════════════════════
        if len(rows_all) >= 2:
            st.markdown('<div class="bp-section"><span class="bp-section-text">④ مقایسه رادار — نمادها</span></div>', unsafe_allow_html=True)
            ...
            radar_m = {
                "تغییر %":      _norm_im([abs(safe_float(r.get("plp",0))) for r in rows_all]),
                "دامنه روز":    _norm_im([safe_float(r.get("pmaxp",0)) - safe_float(r.get("pminp",0)) for r in rows_all]),
                "حجم":          _norm_im([safe_float(r.get("tvol",0)) for r in rows_all]),
                "ارزش":         _norm_im([safe_float(r.get("tval",0)) for r in rows_all]),
                "تعداد معامله": _norm_im([safe_float(r.get("tno",0)) for r in rows_all]),
            }
            cats_r = list(radar_m.keys()) + [list(radar_m.keys())[0]]
            _clrs_r = [blue_im, green_im, gold_im, red_im, "#9b72c8", "#48b8c0"]

            fig_radar_im = go.Figure()
            for i, row in enumerate(rows_all[:6]):
                vals = [radar_m[m][i] for m in radar_m]
                fig_radar_im.add_trace(go.Scatterpolar(
                    r=vals+[vals[0]], theta=cats_r,
                    fill="toself",
                    line=dict(color=_clrs_r[i%6], width=2),
                    fillcolor=f"rgba(91,155,213,{0.06+i*0.01})",
                    name=row.get("commodity","?"),
                ))

            _bg_r  = "#161616" if is_dark else "#e8e8e4"
            _txt_r = "#c0c0c0" if is_dark else "#222220"
            _g_r   = "rgba(255,255,255,0.08)" if is_dark else "rgba(60,60,55,0.1)"
            fig_radar_im.update_layout(
                polar=dict(
                    bgcolor=_bg_r,
                    angularaxis=dict(tickfont=dict(size=9,color=_txt_r,family="JetBrains Mono"),
                                     linecolor=_g_r, gridcolor=_g_r),
                    radialaxis=dict(visible=True, range=[0,100],
                                    tickfont=dict(size=7,color=_txt_r),
                                    gridcolor=_g_r, linecolor=_g_r,
                ...
                margin=dict(l=60,r=60,t=40,b=40),
                title=dict(text="RADAR — مقایسه نمادها (نرمال‌شده)",
                           font=dict(color=_txt_r,size=11),x=0.5),
                legend=dict(bgcolor="rgba(0,0,0,0.3)" if is_dark else "rgba(240,240,237,0.85)",
        ...
        for row in rows_all:
            ...
            if plp_v >= 4:    signals.append("📈 صعود قوی"); score += 2
            elif plp_v >= 1:  signals.append("↗ صعود ملایم"); score += 1
            elif plp_v <= -4: signals.append("📉 ریزش قوی");  score -= 2
            elif plp_v <= -1: signals.append("↘ ریزش ملایم"); score -= 1

            if pmax_v > 0 and pmin_v > 0:
                pos_rng = (pl_v - pmin_v) / (pmax_v - pmin_v + 1e-9)
                if pos_rng > 0.85:   signals.append("🔝 نزدیک سقف روز"); score += 1
                elif pos_rng < 0.15: signals.append("⬇ نزدیک کف روز");   score -= 1

            if qd1_v > 0 and qo1_v == 0:
                signals.append("💚 صف خرید — بدون عرضه"); score += 2
            elif qo1_v > 0 and qd1_v == 0:
                signals.append("🔴 صف فروش — بدون تقاضا"); score -= 2
            elif qd1_v > 0 and qo1_v > 0:
                ratio = qd1_v / (qo1_v + 1e-9)
                if ratio > 3:     signals.append(f"📗 فشار خرید قوی ({ratio:.1f}x)"); score += 1
                elif ratio < 0.33: signals.append(f"📛 فشار فروش قوی ({ratio:.1f}x)"); score -= 1

            if tno_v > 1000: signals.append("🔥 حجم بالا")

            if score >= 3:    verdict, vc = "🟢 خرید",       green_im
            elif score >= 1:  verdict, vc = "💛 تمایل خرید", gold_im
            elif score <= -3: verdict, vc = "🔴 فروش",        red_im
            elif score <= -1: verdict, vc = "🟠 تمایل فروش", "#e8945a" if is_dark else "#7a3a00"
            else:             verdict, vc = "⚪ خنثی",        muted_im

            reasons = " | ".join(signals) if signals else "بدون سیگنال واضح"

            st.markdown(
                f"<div style='display:flex;align-items:center;gap:12px;"
                f"padding:0.55rem 0.9rem;margin-bottom:0.35rem;"
                f"background:{card_im};border:1px solid {border_im};"
                f"border-left:4px solid {vc};border-radius:4px'>"
                f"<span style='font-family:JetBrains Mono,monospace;font-size:0.78rem;"
                f"font-weight:700;color:var(--accent);min-width:80px'>{commodity}</span>"
                f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                f"color:{muted_im};min-width:80px'>{ime_fmt(pl_v)} ریال</span>"
                f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                f"font-weight:700;color:{vc};min-width:65px'>{plp_v:+.2f}%</span>"
                f"<span style='font-size:0.75rem;font-weight:700;min-width:110px'>{verdict}</span>"
                f"<span style='font-size:0.62rem;color:{muted_im};flex:1'>{reasons}</span>"
                f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                f"font-weight:700;color:{vc}'>امتیاز: {score:+d}</span>"
                f"</div>",
                unsafe_allow_html=True
            )

        st.caption("⚠ این سیگنال‌ها صرفاً بر اساس داده لحظه‌ای API و قوانین ساده هستند و توصیه سرمایه‌گذاری نیستند.")

    else:
        st.info("ابتدا داده‌ها را بارگزاری کنید یا داده نمونه را فعال کنید.")

    # ══ ⑥ تنظیمات API ══════════════════════════════════════════
    with st.expander("⚙ تنظیمات API — تست endpoint های مختلف", expanded=False):
        st.markdown(
            f"<div style='background:{card_im};border:1px solid {border_im};border-radius:4px;"
            f"padding:0.8rem 1rem;margin-bottom:0.6rem;font-size:0.65rem;"
            f"font-family:JetBrains Mono,monospace;color:{muted_im};line-height:2'>"
            f"<strong style='color:var(--accent)'>API Base:</strong> {IME_BASE_URL}<br>"
            f"<strong style='color:var(--accent)'>Key:</strong> {IME_API_KEY[:12]}…{IME_API_KEY[-4:]}<br>"
            f"<strong style='color:var(--accent)'>TTL Cache:</strong> 60s (last-changes) | 300s (history)"
            f"</div>",
            unsafe_allow_html=True
        )

        test_ep = st.text_input(
            "endpoint برای تست",
            value="/commodity-deposit-certificate/last-changes",
            key="ime_test_ep"
        )
        test_params_str = st.text_input(
            "پارامترها (JSON)", value="{}",
        ...
        if st.button("🧪 تست endpoint", use_container_width=True, key="ime_ep_test"):
            try:
                params_d = _json_ime.loads(test_params_str) if test_params_str.strip() else {}
            except Exception:
                params_d = {}
            with st.spinner("در حال فراخوانی API…"):
                result = ime_fetch(test_ep, params_d if params_d else None)
            if result:
                if "_error" in result:
        ...
        st.markdown(
            f"<div style='margin-top:0.5rem;font-size:0.62rem;color:{muted_im};line-height:2'>"
            f"📌 <strong>Endpoints اصلی:</strong><br>"
            f"· <code>/commodity-deposit-certificate/last-changes</code> — تمام نمادها (زنده)<br>"
            f"· <code>/commodity-deposit-certificate/{{contract_code}}/history</code> — تاریخچه<br>"
            f"📌 <strong>فیلدهای کلیدی:</strong> pl, plc, plp, pmax, pmin, tno, tvol, tval, pd1-3, qd1-3, po1-3, qo1-3"
            f"</div>",
            unsafe_allow_html=True
        )
