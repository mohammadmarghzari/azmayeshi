...
if is_all_assets_valid(all_assets):
    ...
    # نمودار مقایسه دلاری سود(بازده) سبک‌ها
    col_gains = ['سالانه', 'سه‌ماهه', 'دوماهه', 'یک‌ماهه']
    st.subheader(f"📊 نمودار مقایسه سود دلاری سبک‌ها (بر اساس مبلغ {capital_for_gain:.0f} $)")
    ...
    for style in style_dict:
        fig_gains.add_trace(go.Bar(
            ...
            y=gains_df[style],
            name=style,
            marker_color=color_map[style]
        ))
    fig_gains.update_layout(
        barmode='group', xaxis_title='بازه', yaxis_title='میزان سود تخمینی ($)',
        title='سود دلاری سبک‌های مختلف در بازه‌های زمانی متفاوت', font_family="Vazirmatn", title_font_size=22
    )
    st.plotly_chart(fig_gains, use_container_width=True)

    # باقی بخش‌های کد (جدول مقایسه، مرز کارا، پیش‌بینی قیمت و ...) بدون تغییر مانند نسخه قبلی شما قابل استفاده است
    # دیگر بخش‌ها را مطابق قبل اضافه کنید اگر لازم است تکرار نشود
