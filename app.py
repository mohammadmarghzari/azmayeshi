...
def max_gain(returns):
    annual_returns = returns.resample('Y').sum() * 100 if hasattr(returns, 'index') and isinstance(returns.index, pd.DatetimeIndex) else pd.Series(returns).rolling(252).sum() * 100
    return annual_returns.max() if not annual_returns.empty else 0
...
# ==================== صفحه اصلی ====================
st.set_page_config(page_title="Portfolio360 Ultimate – ریکاوری تایم + بهترین/بدترین", layout="wide")
