...
def calculate_portfolio():
    ...
    # محاسبه عملکرد پایه
    base_return = np.dot(mean_ret, weights) * 100
    base_risk = np.sqrt(np.dot(weights.T, np.dot(cov_mat, weights))) * 100
    base_sharpe = (base_return/100 - rf) / (base_risk/100) if base_risk > 0 else 0
    ...
    adjusted_return = base_return - option_cost
    adjusted_risk = base_risk
    if "Put" in st.session_state.option_strategy:
        adjusted_risk *= 0.7
    elif "Call" in st.session_state.option_strategy:
        adjusted_risk *= 1.1
    adjusted_sharpe = (adjusted_return/100 - rf) / (adjusted_risk/100) if adjusted_risk > 0 else 0

    # نمایش نتایج (بر اساس آپشن)
    is_option_active = st.session_state.option_strategy != "بدون آپشن"
    return_label = "بازده (با آپشن)" if is_option_active else "بازده"
    risk_label = "ریسک (با آپشن)" if is_option_active else "ریسک"
    sharpe_label = "شارپ (با آپشن)" if is_option_active else "شارپ"

    st.success(f"سبک: **{st.session_state.selected_style}** | آپشن: **{opt['name']}**")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric(return_label, f"{adjusted_return:.2f}%")
    c2.metric(risk_label, f"{adjusted_risk:.2f}%")
    c3.metric(sharpe_label, f"{adjusted_sharpe:.3f}")
