...
hedge_table = pd.DataFrame([
    ["Barbell طالب (۹۰/۱۰)", "۹۰٪ ایمن (طلا+تتر) + ۱۰٪ خیلی پرریسک\nایده اصلی نسیم طالب از ۲۰۰۷ — در ۲۰۰۸ بیش از ۱۰۰٪ سود کرد وقتی بازار ۵۰٪ ریخت", "همیشه — مخصوصاً وقتی نمی‌دانید آینده چه می‌شود", "۴۵٪", "۴۵٪", "۱۰٪"],
    ["Tail-Risk طالب", "حفاظت در فاجعه + خرید Put دور\nطالب در مارس ۲۰۲۰ بیش از ۴۰۰۰٪ سود کرد!", "وقتی احتمال جنگ، کرونا یا سقوط دلار بالاست", "۳۵٪", "۳۵٪", "۵٪"],
    ["Antifragile طالب", "هرچه آشوب بیشتر، سود بیشتر!\nطلا + بیت‌کوین + آپشن Long Volatility", "ایران ۱۴۰۴ با تورم، تحریم و نوسان شدید", "۴۰٪", "۲۰٪", "۴۰٪"],
    ["طلا + تتر (ترکیبی)", "بهترین دفاع در شرایط فعلی ایران\nتعادل عالی بین حفاظت و رشد", "سرمایه‌گذار معمولی ایرانی", "۱۵٪", "۱۰٪", "۲۰٪"],
    ["حداقل هج", "فقط یک لایه حفاظتی کوچک\nآزادی عمل بالا در بازار صعودی", "سرمایه‌گذار ریسک‌پذیر", "۱۰٪", "۰٪", "۴۰٪"],
    ["بدون هجینگ (کاملاً آزاد)", "هیچ محدودیتی ندارد — فقط برای کسانی که تحمل ضرر ۵۰–۸۰٪ رو دارن", "افق بلندمدت + بخش زیادی از دارایی جداگانه در طلا/دلار", "۰٪", "۰٪", "۱۰۰٪"],
], columns=["استراتژی", "چرا این استراتژی؟", "مناسب برای", "حداقل طلا", "حداقل تتر/دلار", "حداکثر بیت‌کوین"])

# ==================== جدول کامل آپشن ====================
option_table = pd.DataFrame([
    ["بدون آپشن", "ساده و بدون هزینه — هیچ حفاظتی ندارد", "بازار صعودی قوی — ریسک‌پذیرها", "۰٪"],
    ["Protective Put", "مثل بیمه کامل پرتفوی\nحتی در سقوط ۵۰٪ ضرر محدود می‌شه", "خواب راحت در بحران — کسانی که نمی‌خوان ضرر سنگین ببینن", "+۴.۸٪"],
    ["Collar (تقریباً رایگان)", "هج کم‌هزینه با فروش Call\nسود بالا محدود می‌شه ولی هزینه Put صفر", "بازار متوسط و پرنوسان — بهترین تعادل", "+۰.۴٪"],
    ["Covered Call", "درآمد ماهانه از فروش Call روی دارایی‌ها", "بازار رنج یا صعودی ملایم", "−۳.۲٪ (درآمد)"],
    ["Cash-Secured Put", "فروش Put → اگر ریخت با تخفیف می‌خرید", "وقتی به کف قیمت اعتقاد دارید", "−۲.۹٪ (درآمد)"],
    ["Tail-Risk Put", "خرید Put خیلی دور برای فاجعه\nهزینه کم، حفاظت کامل", "کرونا، جنگ، فروپاشی — مثل طالب", "+۲.۱٪"],
    ["Iron Condor", "درآمد بالا از ثبات بازار\nوقتی قیمت رنج می‌زنه پول می‌گیرید", "بازار آرام و بدون جهت", "−۵.۵٪ (درآمد)"],
...
def calculate_portfolio():
    ...
    cols[1].metric("ریسک سالانه", f"{port_risk:.2f}%")
    cols[2].metric("نسبت شارپ", f"{sharpe:.3f}")
    cols[3].metric("بدترین افت", f"{current_dd:.1f}%")
    cols[4].metric("وضعیت فعلی", "خطرناک!" if current_dd < -20 else "هشدار" if current_dd < -10 else "عالی")
    ...
    # حالت "چه می‌شد اگر"
    st.markdown("### چه می‌شد اگر؟ (What-if)")
    col1, col2, col3 = st.columns(3)
    initial = col1.number_input("سرمایه اولیه (میلیون تومان)", 10, 10000, 100)
    years = col2.selectbox("چند سال پیش شروع کرده بودید؟", [1, 3, 5, 7, 10])
    ...
    for r in backtest:
        value *= (1 + r)
        if backtest.index.get_loc(r.name) % 21 == 0:  # هر ماه
            value += monthly
...
# ==================== صفحه اصلی ====================
st.set_page_config(page_title="Portfolio360 Ultimate – جدول کامل", layout="wide")
st.markdown("<h1 style='text-align: center; color: #00d2d3;'>Portfolio360 Ultimate</h1>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align: center;'>اپ حرفه‌ای سرمایه‌گذاری ایرانی – با جدول‌های کامل</h3>", unsafe_allow_html=True)
...
st.balloons()
st.caption("Portfolio360 Ultimate – نسخه نهایی با جدول‌های کامل + چه می‌شد اگر + هشدار سقوط | با عشق برای ایران")
