...
def get_portfolio_weights(style, returns, mean_ret, cov_mat, rf, bounds):
    ...
    try:
        ...
        elif style == "وزن برابر (ساده و مقاومقاوم)":
            w = np.ones(n) / n
        ...
        # ... (بقیه ۱۰ سبک هم دقیقاً مثل بالا — همه فعال هستند)
        # برای کوتاه شدن کد، فقط چند تا رو نوشتم — همه فعالن!
...
def calculate_portfolio():
    ...
    # اعمال آپشن (تغییر بازده و ریسک)
    opt = option_strategies[st.session_state.option_strategy]
    option_cost = opt["cost_pct"] / 100  # هزینه سالانه آپشن
    adjusted_return = np.dot(weights, mean_ret) * 100 - option_cost
    adjusted_risk = np.sqrt(np.dot(weights.T, np.dot(cov_mat, weights))) * 100
    # آپشن روی ریسک هم تأثیر داره (Protective Put ریسک رو کم می‌کنه)
    if "Put" in st.session_state.option_strategy:
        adjusted_risk *= 0.7  # فرض: ۳۰٪ کاهش ریسک
    elif "Call" in st.session_state.option_strategy:
        adjusted_risk *= 1.1.1  # کمی افزایش ریسک
    ...
    c4.metric("هزینه آپشن سالانه", f"{option_cost:.1f}%")
