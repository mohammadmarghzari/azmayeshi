...
if "prices" not in st.session_state or st.session_state.prices is None:
    ...
    # =============================================================================
    st.markdown('<div class="section-header">🎯 تن��یمات پرتفوی و تخصیص سرمایه</div>', unsafe_allow_html=True)
    ...
    with col_m3:
        st.metric("نسبت ��ارپ", f"{p_sharpe:.3f}")
    with col_m4:
    ...
    # MONTE CARLO FORECAST
    # =============================================================================
    st.markdown('<div class="section-header">🔮 پیش‌بینی قیمت (Monte Carlo)</div>', unsafe_change_html=True)
