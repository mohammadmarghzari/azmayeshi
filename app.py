...
class PortfolioOptimizer:
    ...
    def portfolio_volatility(self, weights):
        return np.sqrt(np.dot(weights.T, np.dot(self.cov_mat, weights)))
    ...
    def inverse_volatility(self):
        """معکوس نوسان"""
        vols = np.sqrt(np.diag(self.cov_mat))
            ...
            while len(clusters) > 0:
                ...
                for i in range(0, len(clusters), 2):
                    if i + 1 < len(clusters):
                        ...
                        cov1 = cov.iloc[c1, c1]
                        cov2 = cov.iloc[c2, c2]
                        vol1 = np.sqrt(np.linalg.pinv(cov1).sum())
                        vol2 = np.sqrt(np.linalg.pinv(cov2).sum())
                        alpha = 1 - vol1 / (vol1 + vol2)
                        w[c1] *= alpha
                        w[c2] *= 1 - alpha
            return w
    ...
    def kelly_criterion(self, half_kelly=True):
        ...
        try:
            ...
            kelly_weights = cov_inv @ (self.mean_ret.values - self.rf_rate)
    ...
    def black_litterman(self, P=None, Q=None, omega=None, tau=0.025):
        """بلک-لیترمن"""
        try:
            pi = self.rf_rate + np.linalg.inv(np.linalg.inv(self.cov_mat.values) @ (self.mean_ret.values - self.rf_rate))
            ...
            def neg_bl_sharpe(w):
                ...
                p_vol = np.sqrt(np.dot(w.T, np.dot(self.cov_mat, w)))
                return -(p_ret - self.rf_rate) / p_vol
    ...
    def resampled_efficiency(self, n_sims=100):
        ...
        for _ in range(n_sims):
            ...
            def neg_sharpe(w):
                ...
                p_vol = np.sqrt(np.dot(w.T, np.dot(sample_cov, w)))
                return -(p_ret - self.rf_rate) / p_vol if p_vol > 0 else 0
