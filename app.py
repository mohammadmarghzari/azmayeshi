...
# ==================== دریافت قیمت زنده (اولویت: BrsApi → tgju → yfinance → پیش‌فرض) ====================
...
def get_iran_prices():
    ...
    # مرحله ۱: BrsApi.ir (اگر فعال شد، بهترین گزینه)
    try:
        api_key = "Bs76KjzbGZbipLtarU5SRrH7vCeufSqQ"
        url = f"https://api.brsapi.ir/v1/rates?api_key={api_key}"
        r = requests.get(url, headers=headers, timeout=8)
        if r.status_code == 200:
            data = r.json()
            usd_toman = float(data.get("US Dollar", {}).get("sell", 119700))
            gold_toman = float(data.get("Gerami", {}).get("sell", 122480))
            return usd_toman * 10, gold_toman * 10  # به ریال
    except:
        pass  # ساکت ادامه بده

    # مرحله ۲: tgju.org (همیشه کار می‌کنه)
    try:
        # دلار
        ...
        return dollar_toman * 10, gold_toman * 10
    except:
        pass

    # مرحله ۳: yfinance (fallback جهانی)
    try:
    ...
    # مرحله ۴: مقادیر واقعی پیش‌فرض (آذر ۱۴۰۴)
