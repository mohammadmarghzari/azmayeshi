                ...
                for t in tickers:
                    ...
                    if df is not None:
                        if df.empty or df["Price"].isna().all():
                            asset_read_errors.append(f"{t}: داده دریافتی معتبر نیست.")
                        else:
                            df['Date'] = pd.to_datetime(df['Date'])
                            new_downloaded.append((t, df))
                            msg(f"داده {t} با موفقیت دانلود شد.", "success")
                    else:
                        asset_read_errors.append(f"{t}: {err}")
...
if is_all_assets_valid(all_assets):
    ...
    # ---------------- افزونه: سبک‌های دیگر ----------------
    st.subheader(":rocket: پورتفوی بهینه براساس سایر سبک‌ها")
    bounds = [(asset_min_weights.get(name,0)/100, asset_max_weights.get(name,100)/100) for name in asset_names]
    # مارکویتز - مینیمم واریانس
    w_mvp = opt_min_variance(mean_returns, cov_matrix, bounds)
    if w_mvp is not None:
        port_ret = np.dot(w_mvp, mean_returns)
        port_risk = np.sqrt(np.dot(w_mvp.T, np.dot(cov_matrix, w_mvp)))
        st.markdown(f":pushpin: پرتفو مینیمم واریانس (کلاسیک): بازده **{port_ret:.2%}** | ریسک **{port_risk:.2%}**")
        for i, name in enumerate(asset_names):
            st.markdown(f"▫️ وزن {name}: {w_mvp[i]*100:.2f}%")
    # مارکویتز - ماکزیمم شارپ
    w_sharpe = opt_max_sharpe(mean_returns, cov_matrix, rf/100, bounds)
    if w_sharpe is not None:
        port_ret = np.dot(w_sharpe, mean_returns)
        port_risk = np.sqrt(np.dot(w_sharpe.T, np.dot(cov_matrix, w_sharpe)))
        if port_risk!=0:
            port_sharpe = (port_ret - rf/100)/port_risk
        else:
            port_sharpe = np.nan
        st.markdown(f":star: پرتفو ماکزیمم شارپ (کلاسیک): بازده **{port_ret:.2%}** | ریسک **{port_risk:.2%}** | شارپ **{port_sharpe:.2f}**")
        for i, name in enumerate(asset_names):
            st.markdown(f"▪️ وزن {name}: {w_sharpe[i]*100:.2f}%")
    # وزن برابر
    w_eq = equally_weighted_weights(len(asset_names))
    port_ret = np.dot(w_eq, mean_returns)
    port_risk = np.sqrt(np.dot(w_eq.T, np.dot(cov_matrix, w_eq)))
    st.markdown(f":balance_scale: پورتفو وزن برابر: بازده **{port_ret:.2%}** | ریسک **{port_risk:.2%}**")
    for i, name in enumerate(asset_names):
        st.markdown(f"▪️ وزن {name}: {w_eq[i]*100:.2f}%")
    # ---------------- /افزونه سبک‌های دیگر ----------------
    ...
    fig_w.add_trace(go.Bar(x=asset_names, y=w_mvp*100 if w_mvp is not None else [0]*len(asset_names), name='مینیمم واریانس', marker_color="#8e44ad"))
    fig_w.add_trace(go.Bar(x=asset_names, y=w_sharpe*100 if w_sharpe is not None else [0]*len(asset_names), name='ماکزیمم شارپ', marker_color="#34495e"))
    fig_w.add_trace(go.Bar(x=asset_names, y=w_eq*100, name='وزن برابر', marker_color="#7ed6a5"))
    fig_w.update_layout(barmode='group', title="مقایسه وزن دارایی‌ها در سبک‌های مختلف", font_family="Vazirmatn",
                        xaxis_title='دارایی', yaxis_title='وزن (%)', legend_title='سبک', title_font_size=20)
    st.plotly_chart(fig_w, use_container_width=True)

    # ... *** ادامه باقی کد به همان صورت قبل است (مرز کارا، married put، سیمولیشن قیمت و ... ) ***
    ...
    for name, info in insured_assets.items():
        is_valid, put_msg = validate_married_put_params(info)
        if not is_valid:
            st.warning(f"{name}: {put_msg}")
            continue
        ...
        fig2.add_trace(go.Scatter(x=[max_x], y=[max_pnl], mode='markers+text', marker=dict(color='green', size=14),
                                  ...
                                  showlegend=False))
        fig2.update_layout(title=f'نمودار سود و زیان (Married Put) برای {name}', font_family='Vazirmatn',
        ...
        if fig_bytes is not None:
            st.download_button(f"دانلود نمودار Married Put برای {name}", fig_bytes, file_name=f"married_put_{name}.png")
        else:
