...
# برای نمودار دایره‌ای، وزن‌های کوچک با "سایر" جمع می‌شوند
...
# محاسبه پارامترهای پرتفو در بازه‌های مختلف
def portfolio_stats(weights, mean_returns, cov_matrix, returns, rf, annual_factor):
    # weights: آرایه
    # returns: دیتافریم بازده‌ها (resampled)
    # annual_factor: چگالی سالانه مثلاً 12 ماهانه
    # برگشتی: دیکشنری {"annual", "3m", "2m", "1m"} + sharpe, sortino
    # محاسبه ریسک و بازده برای 1 سال و 3 و 2 و 1 ماه
    mean_r = mean_returns
    stds = np.sqrt(np.diag(cov_matrix))
    cov = cov_matrix

    # نسبت‌های سالانه
    port_ann_return = np.dot(weights, mean_r)
    port_ann_vol = np.sqrt(np.dot(weights.T, np.dot(cov, weights)))
    downrets = returns.copy()
    downrets[downrets > 0] = 0
    ...
    stats['port_ann_return'] = port_ann_return
    stats['port_ann_vol'] = port_ann_vol
    return stats
...
if is_all_assets_valid(all_assets):
    ...
    # نمایش اطلاعات پرتفوها برای هر سبک + PieChart
    st.subheader(":rocket: اطلاعات سبد (در همه سبک‌ها)")
    for style, weights in style_dict.items():
        stats = portfolio_stats(
            weights, mean_returns, cov_matrix, returns, rf, annual_factor
        )
        st.markdown(f"##### <span style='color:{color_map[style]}'>سبک: {style}</span>", unsafe_allow_html=True)
        st.write(
            f"""
سالانه:
- سود مورد انتظار: **{stats['سالانه']['return']*100:.2f}%**
- ریسک (انحراف معیار): **{stats['سالانه']['vol']*100:.2f}%**
سه‌ماهه:
- سود مورد انتظار: **{stats['سه‌ماهه']['return']*100:.2f}%**
- ریسک: **{stats['سه‌ماهه']['vol']*100:.2f}%**
دوماهه:
- سود مورد انتظار: **{stats['دوماهه']['return']*100:.2f}%**
- ریسک: **{stats['دوماهه']['vol']*100:.2f}%**
یک‌ماهه:
- سود مورد انتظار: **{stats['یک‌ماهه']['return']*100:.2f}%**
- ریسک: **{stats['یک‌ماهه']['vol']*100:.2f}%**

نسبت شارپ: **{stats['sharpe']:.2f}**
نسبت سورتینو: **{stats['sortino']:.2f}**
            """
        )
    ...
    # باقی ابزار، پیش‌بینی قیمت، married put و ... بدون تغییر باقی بماند
else:
