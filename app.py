...
# ==================== استراتژی‌های هجینگ (نسیم طالب + کلاسیک) ====================
hedge_strategies = {
...
def calculate_portfolio():
    ...
    fig_pie.update_layout(title="تخصیص دارایی پرتفوی")

    # نمودار ۲: بازده vs ریسک
    fig_scatter = go.Figure()
    for i, asset in enumerate(asset_names):
        fig_scatter.add_trace(go.Scatter(
            x=[np.sqrt(cov_mat.iloc[i,i]) * 100 * np.sqrt(252)],
            y=[mean_ret.iloc[i] * 100],
            mode='markers+text',
            marker=dict(size=12),
            name=asset,
            text=[asset],
    ...
    fig_scatter.add_trace(go.Scatter(
        x=[port_risk], y=[port_return],
        mode='markers',
        marker=dict(color='red', size=20, symbol='star'),
        name="پرتفوی شما"
    ))
    fig_scatter.update_layout(
        title="بازده در مقابل ریسک (هر نقطه = یک دارایی)",
        xaxis_title="ریسک سالانه (%)",
        yaxis_title="بازده سالانه (%)",
        template="plotly_dark"
    )

    # نمودار ۳: تکامل پرتفوی
    port_value = (1 + returns.dot(weights)).cumprod()
    fig_cum = px.line(port_value, title="تکامل ارزش پرتفوی (Backtest)")
    fig_cum.update_layout(yaxis_title="ارزش نسبی (شروع = ۱)", xaxis_title="زمان")

    # نمایش نتایج
    st.success(f"استراتژی: **{hedge}** + **{opt}**")
...
# ==================== صفحه اصلی ====================
st.set_page_config(page_title="Portfolio360 Ultimate – با نمودارهای بصری", layout="wide")
st.markdown("<h1 style='text-align: center; color: #00d2d3;'>Portfolio360 Ultimate</h1>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align: center;'>اپ حرفه‌ای سرمایه‌گذاری ایرانی – با نمودارهای بصری</h3>", unsafe_allow_html=True)
...
tickers = st.sidebar.text_input("نمادها", value="BTC-USD, GC=F, USDIRR=X, ^GSPC, ETH-USD")
...
st.balloons()
st.caption("Portfolio360 Ultimate – نسخه نهایی با نمودارهای بصری | با عشق برای ایران")
