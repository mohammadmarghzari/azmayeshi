...
def download_data(tickers_str, period="max"):
    ...
    for t in tickers:
        try:
            df = yf.Ticker(t).history(period=period if period else "max", auto_adjust=True)
...
st.markdown("<h3 style='text-align: center;'>اپ حرفه‌ای سرمایه‌گذاری ایرانی – با جدول کامل، مرز کارا و «چه می‌شد اگر»</h3>", unsafe_allow_html=True)
