...
# کلید API BrsApi.ir (در تولید از st.secrets استفاده کن)
BRS_API_KEY = "Bs76KjzbGZbipLtarU5SRrH7vCeufSqQ"

# ==================== دریافت قیمت زنده از BrsApi.ir (اولویت اول) یا tgju.org (fallback) ====================
...
def get_iran_prices():
    ...
    # اولویت ۱: BrsApi.ir
    try:
        url = "https://api.brsapi.ir/v1/rates"
        params = {"api_key": BRS_API_KEY}
        r = requests.get(url, params=params, headers=headers, timeout=10)
        if r.status_code == 200:
            data = r.json()
            # دلار آزاد (sell)
            usd_toman = float(data.get("US Dollar", {}).get("sell", 119700))
            usd_irr = usd_toman * 10  # به ریال
            
            # طلای ۱۸ عیار (Gerami - هر گرم)
            gold_toman = float(data.get("Gerami", {}).get("sell", 122480))
            gold_irr = gold_toman * 10  # به ریال
            
            return usd_irr, gold_irr
    except Exception as e:
        st.warning(f"API BrsApi.ir در دسترس نیست: {e}. fallback به tgju.org")
    
    # Fallback: tgju.org scraping
    try:
        # دلار
        ...
        return 1197000, 12248000  # مقادیر پیش‌فرض واقعی
...
option_data = {
    ...
    "توضیح کامل": [
        ...
        "درآمد ماهانه ثابت: Call روی دارایی‌های خودتان می‌فروشید و پول می‌گیرید. دلیل: سود اضافی در بازار رنج یا صعودی ملایم، بدون ریسک زیاد.",
