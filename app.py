...
premium_pct = st.slider(
    "هزینه تقریبی آپشن (% سرمایه)",
    0.0, 10.0, 3.0, 0.1
)

original_risk = portfolio_risk(weights, cov)
risk_reduction = premium_pct * (0.45 if hedge_type != "بدون هج" else 0)
...
premium = st.number_input("Premium پرداختی", value=4.0)
...
پرتفوی ۳۶۰ — نسخه نهایی موبایل‌پسند  
<br>
Cloud-Ready • Options • Hedging
</p>
""", unsafe_allow_html=True)
