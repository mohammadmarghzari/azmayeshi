...
# ------------------ Utils -------------------
def calculate_recovery_time(port_returns):
    ...
    for i in range(1, len(cum_returns)):
        if cum_returns.iloc[i] < running_max.iloc[i-1]:
            if not in_drawdown:
                in_drawdown = True
                drawdown_start = i-1
        elif in_drawdown and cum_returns.iloc[i] >= running_max.iloc[drawdown_start]:
            in_drawdown = False
...
if is_all_assets_valid(all_assets):
    ...
    for style, weights in style_dict.items():
        ...
        recovery_time = calculate_recovery_time(port_returns)
        ...
        for (p_label, p_mult) in periods:
            ...
            if p_label == 'سالانه':
                ...
                st.write(f"⏳ میانگین زمان ریکاوری: {recovery_time:.2f} دوره ({period})")
