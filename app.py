...
def detect_correlation_regime(returns_df, window_short=30, window_long=126):
    ...
    for i in range(window_long, len(returns_df)):
        cs = returns_df.iloc[i-window_short:i].corr().values
        cl = returns_df.iloc[i-window_long:i].corr().values
...
with tab6:
    ...
    if "prices" not in st.session_state or st.session_state["prices"] is None:
        ...
        if factor_df is not None:
            st.dataframe(factor_df.style.background_gradient(cmap="RdYlGn", subset=["مومنتوم_6ماه","شارپ"])
                                        .background_gradient(cmap="RdYlGn_r", subset=["نوسان_سالانه"]),
