...
def get_iran_prices():
    ...
    try:
        ...
        st.warning("قیمت زنده در دسترس نیست → آخرین مقادیر معتبر")
...
hedge_data = {
    ...
    "مناسب برای": [
        ...
        "بازارهای پرنوسان با شوک‌های ناگهانی (مثل جنگ، تحریم جدید، انتخابات آمریکا). در بحران خودکار حداکثر حفاظت فعال می‌شود.",
        "وقتی نمی‌دانید دنیا کجا می‌رود: ۸۵٪ در دارایی‌های کاملاً ایمن + ۱۵٪ در دارایی‌های خیلی پرریسک (شانس سود انفجاری).",
        "سناریوهای آخرالزمانی: جنگ جهانی، فروپاشی دلار، کرونا جدید. حداکثر حفاظت حتی اگر بقیه دنیا نابود شود.",
        "سرمایه‌گذار ریسک‌پذیر که فقط می‌خواهد یک لایه حفاظتی کوچک داشته باشد. مناسب بازار صعودی پایدار.",
...
option_data = {
    ...
    "توضیح کامل": [
        "بدون هیچ پوشش آپشنی – ساده و بدون هزینه اضافی.",
        "مثل بیمه کامل پرتفوی: حتی اگر بازار ۵۰٪ ریخت، ضرر شما محدود است. مناسب کسانی که خواب راحت می‌خواهند.",
        "هج تقریباً رایگان: با فروش Call سود بالقوه را محدود می‌کنید تا Put تقریباً مجانی شود.",
        "درآمد ماهانه ثابت: Call روی دارایی‌های خودتان می‌فروشید و پول می‌گیرید. مناسب بازار رنج یا صعودی ملایم.",
        "اگر بازار ریخت، با تخفیف می‌خرید: Put می‌فروشید و پول می‌گیرید. مناسب وقتی فکر می‌کنید قیمت پایین‌تر نمی‌آید.",
        "برای فاجعه‌های بزرگ (کرونا، جنگ): Put خیلی دور می‌خرید – هزینه کم، حفاظت کامل.",
        "درآمد بالا از ثبات: وقتی بازار رنج می‌زند، پول زیادی از فروش آپشن می‌گیرید."
    ],
    "هزینه سالانه (%)": [0, "+۴.۸٪", "+۰.۴٪", "−۳.۲٪ (درآمد)", "−۲.۹٪ (درآمد)", "+۲.۱٪", "−۵.۵٪ (درآمد)"],
...
st.markdown("<h3 style='text-align: center;'>اولین اپ حرفه‌ای سرمایه‌گذاری ایرانی با راهنما و قیمت زنده</h3>", unsafe_allow_html=True)
...
st.balloons()
st.caption("Portfolio360 Ultimate – نسخه نهایی ۱۴۰۴ | ساخته شده با عشق برای ایران")
