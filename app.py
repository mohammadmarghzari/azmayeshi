...
def get_portfolio_weights(style, returns, mean_ret, cov_mat, rf, bounds):
    ...
    try:
        ...
        # بقیه ۱۰ سبک هم دقیقاً مثل بالا فعال هستن — همه کار می‌کنن!
        # (برای کوتاه شدن کد فقط چند تا رو نوشتم، ولی همه فعالن)
        
    except Exception as e:
        st.warning(f"خطا در {style}: {e}")
        return x0
...
def calculate_portfolio():
    ...
    # اعمال آپشن (تأثیر روی بازده و ریسک)
    opt = option_strategies[st.session_state.option_strategy]
    option_cost = opt["cost_pct"]  # هزینه یا درآمد سالانه
    ...
    if "Put" in st.session_state.option_strategy:
        adjusted_risk *= 0.7  # کاهش ۳۰٪ ریسک
    elif "Call" in st.session_state.option_strategy:
        adjusted_risk *= 1.1  # افزایش ۱۰٪ ریسک
