    ...
    if rows_all:
        ...
        if sel_row:
            ...
            with c1:
                _ob_rows = ""
                for _i in range(3):
                    _bg = "background:rgba(91,155,213,0.07);" if _i == 0 else ""
                    _fw = "700" if _i == 0 else "500"
                    _qd = ime_fmt(sel_row.get(f"qd{_i+1}", 0))
                    _pd_v = ime_fmt(sel_row.get(f"pd{_i+1}", 0))
                    _po_raw = sel_row.get(f"po{_i+1}", 0)
                    _qo_raw = sel_row.get(f"qo{_i+1}", 0)
                    _po = ime_fmt(_po_raw) if _po_raw else "—"
                    _qo = ime_fmt(_qo_raw) if _qo_raw else "—"
                    _ob_rows += (
                        ...
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                        f"font-weight:{_fw};color:{green_im};text-align:right'>{_pd_v}</span>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:0.68rem;"
                ...
                _best_bid_v = ime_fmt(sel_row.get("pd1", 0))
                _best_ask_raw = sel_row.get("po1", 0)
                _best_ask_v = ime_fmt(_best_ask_raw) if _best_ask_raw else "—"
                _spread_v = ime_fmt(abs(float(_best_ask_raw or 0) - float(sel_row.get("pd1", 0) or 0)))
                _commodity_v = sel_row.get("commodity", "—")
                st.markdown(
                    ...
                    f"📋 عرضه و تقاضا — {_commodity_v}</div>"
                    f"<div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;"
                    ...
                    f"<div><span style='font-size:0.58rem;color:{muted_im}'>بهترین خرید: </span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                    f"font-weight:700;color:{green_im}'>{_best_bid_v}</span></div>"
                    f"<div><span style='font-size:0.58rem;color:{muted_im}'>اسپرد: </span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                    f"font-weight:700;color:{gold_im}'>{_spread_v}</span></div>"
                    f"<div><span style='font-size:0.58rem;color:{muted_im}'>بهترین فروش: </span>"
                    f"<span style='font-family:JetBrains Mono,monospace;font-size:0.72rem;"
                    f"font-weight:700;color:{red_im}'>{_best_ask_v}</span></div>"
                    f"</div></div>",
                    unsafe_allow_html=True
                )
            ...
            with c2:
                ...
                det_rows_html = "".join(
                    ...
                    f"padding:0.2rem 0;border-bottom:1px solid {border_im}'>"
                    f"<span style='font-size:0.63rem;color:{muted_im};font-weight:600;"
                    f"min-width:110px'>{lbl}</span>"
