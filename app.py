...
def calculate_portfolio():
    ...
    cols[4].metric("وضعیت فعلی", "خطرناک!" if current_dd < -20 else "عادی" if current_dd < -10 else "عالی")
    cols[5].metric("زمان ریکاوری", format_recovery(calculate_recovery_time(port_returns)))
    ...
    initial_amount = col1.number_input("سرمایه اولیه (میلیون تومان)", 1, 10000, 100)
    years_ago = col2.selectbox("چند سال پیش شروع کرده بودید؟", [1, 3, 5, 10])
    monthly_invest = col3.number_input("سرمایه‌گذاری ماهانه (میلیون)", 0, 100, 5)

    # محاسبه رشد سرمایه
    full_returns = prices.pct_change().dropna()
    port_daily_return = full_returns.dot(weights)
    backtest = (1 + port_daily_return).cumprod()
    backtest = backtest[-years_ago*252:] if len(backtest) > years_ago*252 else backtest

    if len(backtest) > 0:
        final_value = initial_amount * backtest.iloc[-1]
        for i in range(1, len(backtest)):
            if i % 21 == 0:  # هر ماه
                final_value += monthly_invest
            final_value *= (1 + port_daily_return.iloc[i])

        st.metric("اگر از اون موقع شروع کرده بودید، الان داشتید:", f"{final_value:,.0f} میلیون تومان")
        if final_value > initial_amount * 10:
            st.success("عالی! سرمایه شما بیش از ۱۰ برابر شده!")
        elif final_value > initial_amount * 3:
            st.info("خوبه! سرمایه شما بیش از ۳ برابر شده.")
        else:
            st.warning("متأسفانه سود چشمگیری نداشتید.")

        fig_back = px.line(backtest*initial_amount, title=f"رشد سرمایه از {years_ago} سال پیش")
...
st.markdown("<h3 style='text-align: center;'>اپ حرفه‌ای سرمایه‌گذاری ایرانی – با حالت «چه می‌شد اگر» و هشدار سقوط</h3>", unsafe_allow_html=True)
