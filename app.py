...
# افزونه: بهینه‌سازی مارکویتز کلاسیک
...
if is_all_assets_valid(all_assets):
    ...
    # بهبود پیش‌بینی قیمت برای بازه سه ماهه، دو ماهه و یک ماهه
    ...
    for i, name in enumerate(asset_names):
        ...
        if sigma < 1e-4:
            ...
            continue
        st.markdown(f"#### {name}")
        col_predicts = st.columns(len(prediction_periods))
        for j, (label, future_months) in enumerate(prediction_periods):
            ...
            with col_predicts[j]:
                ...
                st.markdown(f"📈 **میانگین قیمت آتی:** `{future_price_mean:.2f}`")
                st.markdown(f"📊 **بازده مورد انتظار:** `{future_return:.2%}`")
        st.markdown("---")

    # باقی بخش‌های married put، مرز کارا و ... بدون تغییر مطابق قبل
else:
