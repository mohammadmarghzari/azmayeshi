...
def calculate_portfolio():
    ...
    for i in range(num_portfolios):
        ...
        portfolio_return, portfolio_std, _ = portfolio_performance(weights, mean_ret, cov_mat, rf)
        results[0,i] = portfolio_std
        results[1,i] = portfolio_return
        results[2,i] = (portfolio_return - rf) / portfolio_std if portfolio_std > 0 else 0
    ...
    for idx, (name, info) in enumerate(hedge_strategies.items()):
        ...
        try:
            ...
            if res.success:
                ...
                strategy_points[name] = (risk, ret, sharpe_ratio, w)
                fig_ef.add_trace(go.Scatter(
                    x=[risk], y=[ret],
                    mode='markers+text',
                    ...
                    hovertext=f"{name}<br>بازده: {ret:.1f}%<br>ریسک: {risk:.1f}%<br>شارپ: {sharpe_ratio:.2f}"
                ))
        except:
            pass
    ...
    with st.expander("مرز کارا چیست؟ — توضیح ریاضی ساده و کامل", expanded=False):
        ...
        **نقاطرقه‌های رنگی**: موقعیت استراتژی‌های طالب و دیگران روی مرز کارا
...
st.session_state.selectbox("انتخاب هجینگ", options=list(hedge_strategies.keys()))
